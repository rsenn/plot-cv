...
(function (globalObj) {
  ...
  Util.className = function(obj) {
    ...
    if(isObject(proto) && 'constructor' in proto) return Util.fnName(proto.constructor);
  };
  ...
  Util.adapter = function(
    obj,
    getLength = (obj => obj.length, (getKey = ((obj, index) => obj.key(index), (getItem = ((obj, key) => obj.key, (setItem = (obj, index, value) => (obj.index = value)))))))
  ) {
    const adapter = obj && {
  ...
  Util.inspect = (obj, opts = {}) => {
    ...
    if(typeof obj == 'number') {
      ...
      /*|| obj instanceof Function || Util.className(obj) == 'Function'*/ obj = '' + obj;
  ...
  Util.typeOf = function(v) {
    if(isObject(v) && Object.getPrototypeOf(v) != Object.prototype) return `${Util.className(v)}`;
  ...
  isObject = (obj, ...protoOrPropNames) => {
    ...
    let r = obj || false;
    if(!r) console.log('isObject(', obj, ...protoOrPropNames, ...')', ...` = ${!!r}`);
    return r;
  };
  ...
  Util.histogram = /* new Set()*/ (arr, t, out = false ? {} : new Map(), initVal = () => 0, setVal = v => v) => {
    ...
    t = t || defKeyFunc;
    if(isObject(arr) && typeof arr.entries == 'function') arr = arr.entries();
    arr = [...arr];
  ...
  Util.inserter = (dest, next = (k, v) => {}) => {
    ...
    const insert =
      /*dest instanceof Map ||
        dest instanceof WeakMap ||*/ typeof dest.set == 'function' && dest.set.length >= 2 ? (k, v) => dest.set(k, v) : Array.isArray(dest) ? (k, v) => dest.push([k, v]) : (k, v) => (dest.k = v);
  ...
  Util.mapReducer = (setFn, filterFn = (key, value) => true, mapObj = new Map()) => {
    ...
    fn = function ReduceIntoMap(arg, acc = mapObj) {
      if(isObject(arg) && typeof o.reduce == 'function') return arg.reduce((acc, arg) => (Array.isArray(arg) ? arg : Util.members(arg)).reduce(reducer, acc), self.map);
      let c = Util.counter();
    ...
    const nodes = Util.weakMapper(
      (value, path) =>
        new Proxy(value, {
          get(target, key) {
            let prop = value.key;
            if(isObject(prop) || Array.isArray(prop)) return new node([...path, key]);
  ...
  if(!Util.isBrowser()) {
    ...
    PointList.prototype.sym = function() {
      return `${c.text('PointList', 1, 31)}${c.text('(', 1, 36)}${c.text(this.getLength(), 1, 35) + c.code(1, 36)}) [
        ${this.map(({ x, y }) => Util.inspect({ x, y }, { multiline: false, spacing: ' ' })).join(
          ///*Point.prototype.toSource.call(point, { plainObj: true, colors: true })  ||*/ Util.toSource(point, {colors: true }) || point[sym]() ||
  ...
  LineList.toPolygons = (lines, createfn = points => Object.setPrototypeOf(points, PointList.prototype)) => {
    ...
    for(var i = 0; i < lines.length; i++) {
      ...
      while(lines.length && (j < lines.length || linesLength != lines.length)) {
        ...
        if(
          polygon.length >= 3 &&
          ((currentEndPoint.x === nextLine.x1 && currentEndPoint.y === nextLine.y1 && currentStartPoint.x === nextLine.x2 && currentStartPoint.y === nextLine.y2) ||
            (currentStartPoint.x === nextLine.x1 && currentStartPoint.y === nextLine.y1 && currentEndPoint.x === nextLine.x2 && currentEndPoint.y === nextLine.y2))
        ) {
  ...
  */
  ...
  Matrix.prototype.decompose = function(degrees = (false, (useLU = true))) {
    ...
    let sign = Matrix.prototype.scaleSign.call(this);
  ...
  Circle.prototype.transform = function(m) {
    ...
    this.radius = Matrix.prototype.transformWH.call(m, this.radius, this.radius)[0];
    return this;
  };
