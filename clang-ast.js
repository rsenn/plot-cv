...
export class Type {
  ...
  get regExp() {
    return new RegExp(
      `(?:${this.qualType}${this.typeAlias ? '|' + this.typeAlias : ''})`.replace(/\*/g, '\\*'),
      'g'
    );
  }
  get subscripts() {
...
export async function AstDump(file, extraArgs) {
  ...
  errorLines = errorLines.filter((line) => /error:/.test(line));
    const numErrors =
    [...(/^([0-9]+)\s/g.exec(errorLines.find(line => /errors\sgenerated/.test(line))||'0')  || [])][0] || errorLines.length;
