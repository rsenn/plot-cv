...
`;
  ...
  function processFile(file) {
    ...
    try {
      ...
      const getFile = Util.memoize((module) =>
        searchModuleInPath(module, file)
      );
      let imports,
      ...
      alter = alter.map((node) => {
        ...
        console.log(
          `node alter ${node.position.toString()}  => '${to}'   (was '${from}' )`
        );
        return node;
      });
      ...
      log(
        ...
        remove
          .reduce(
            (acc, [i, imp]) => [...acc, imp /*(imp.fromPath),imp.toSource()*/],
            []
          )
...
function makeSearchPath(dirs, extra = 'node_modules') {
  let r = [];
  const addPath = (p) => (
    (p = path.relative(cwd, p)), r.indexOf(p) == -1 && r.push(p)
  );
  let i = 0;
  for (let cwd of dirs) {
