...
this.goog.SWLib = (function () {
  ...
  function broadcastUpdate({ channel: a, cacheName: b, url: c, source: d }) {
    assert.isInstance({ channel: a }, BroadcastChannel), assert.isType({ cacheName: b }, 'string'), assert.isType({ source: d }, 'string'), assert.isType({ url: c }, 'string'), a.postMessage({ type: cacheUpdatedMessageType, meta: d, payload: { cacheName: b, updatedUrl: c } });
  }
  ...
  class BroadcastCacheUpdate {
    ...
    notifyIfUpdated({ first: a, second: b, cacheName: c, url: d }) {
      assert.isType({ cacheName: c }, 'string'), responsesAreSame({ first: a, second: b, headersToCheck: this.headersToCheck }) || broadcastUpdate({ cacheName: c, url: d, channel: this.channel, source: this.source });
    }
  }
    ...
    _parseEntry(a) {
      ...
      switch (typeof a) {
        ...
        case 'object':
          ...
          break;
        default: throw ErrorFactory$7.createError('invalid-revisioned-entry', new Error('Invalid file entry: ' + JSON.stringify(b)));
      }
      return b;
    }
