...
// samples.
int
tick(void* outputBuffer, void* inputBuffer, unsigned int nBufferFrames, double streamTime, RtAudioStreamStatus status, void* dataPointer) {
...
usage(void) {
  ...
  std::cout << "    and randomness = factor between 0 - 1.0 to control grain parameter "
               "randomness.\n\n";
  exit(0);
}
