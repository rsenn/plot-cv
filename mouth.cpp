...
main(int argc, char** argv) {
  ...
  Mat_<uchar> pseudo_hue_bin = binaryThresholding(pseudo_hue_plane, returnImageStats(pseudo_hue_plane));
...
Mat_<Vec3b>
extractFaceROI(Mat_<Vec3b> image, string face_cascade_path) {
...
Mat_<Vec3b>
extractMouthROI(Mat_<Vec3b> face_image) {
 ...
 */
Mat_<Vec3b>
equalizeImage(Mat_<Vec3b> image_BGR) {
  vector<Mat> channels;
  Mat_<Vec3b> image_eq;
...
Mat_<uchar>
transformPseudoHue(Mat_<Vec3b> image) {
...
pair<double, double>
Stats(Mat_<double> pseudo_hue_plane) {
  double Hmax = DBL_MIN;
  double Hmin = DBL_MAX;
...
Mat_<uchar>
transformCIELAB(Mat_<Vec3b> image_BGR) {
...
Mat_<uchar>
transformLUX(Mat_<Vec3b> image_BGR) {
...
Mat_<uchar>
transformModifiedLUX(Mat_<Vec3b> image_BGR) {
...
pair<double, double>
returnImageStats(const Mat_<uchar>& image) {
...
Mat_<uchar>
binaryThresholding(const Mat_<uchar>& image, const pair<double, double>& stats) {
