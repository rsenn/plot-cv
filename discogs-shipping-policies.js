...
g.setHandler = (
  ...
  findAll("select, input").forEach((e) =>
    ["change", "keydown"].forEach((n) => (e["on" + n] = fn))
  );
...
g.getElement = (query, root) =>
  typeof query == "string" ? find(query, root) : query;
...
g.toStyle = getElementFn((element) => {
  ...
  return (...args) =>
    args.length >= 2 ? e.style.setProperty(...args) : fn(...args);
});
  ...
  (obj = {}) =>
    new Proxy(obj, {
      get: (target, prop, receiver) =>
        getSetFunction(prop) ?? Reflect.get(target, prop, receiver),
      ownKeys: (target) => getSetFunction() ?? Reflect.ownKeys(target),
    });
g.propertyProxy = (elem, obj = {}) =>
  chainFn1(toStyle, propertyProxyFn)(elem)(obj);

g.getCols = (row, pred = (e) => !e.disabled) =>
  findAll("input, select", row).filter(pred);
...
g.getOptions = (element, t = (a) => a) =>
  [...getElement(element ?? find("select")).children]
    .filter((e) => /^option/i.test(e.tagName))
    .map(t);
...
g.getOptionEntries = (e) =>
  getOptions(e, (a) => [a.getAttribute("value"), (a.innerText + "").trim()]);
...
g.getSelectOption = (e = find("select")) =>
  e.options ? e.options[e.selectedIndex] : null;
g.getSelectText = (e = find("select")) => g.getSelectOption(e)?.innerText;
g.getValue = (e) =>
  /^select$/i.test(e.tagName) ? getSelectValue(e) : getInputValue(e);
g.getText = g.trimFn((e) =>
  ((
    { select: getSelectText, input: getInputLabel }[e.tagName.toLowerCase()] ??
    getCardText
  )(e))
); //  /^select$/i.test(e.tagName) ? getSelectText(e) : getInputLabel(e)

g.getSelections = (e) =>
  getOptions(e, (t = (o, i) => [o.innerText, /*o.value,*/ o.selected]));
g.setSelection = (e, i) => (getElement(e).selectedIndex = i); //getSelections(e, o => [e,o.innerText,o.value,i]).findIndex(([e,...s]) => s.some(t => t+'' == i+''));
g.findOption = (e) =>
  makeExprFn((expr) =>
    getOptionEntries(e).findIndex((a, i) => a.some((t) => expr.test(t)))
  );
g.filterOptions = (e) =>
  makeExprFn((expr) =>
    getOptionEntries(e).filter((a, i) => a.some((t) => expr.test(t)))
  );
...
g.getPriceText = (price = find(".price-row-price > span")) =>
  (price?.innerText ?? "").trim();

g.getPrice = (price = find(".price-row-price > span")) =>
  parseNumber(g.getPriceText(price));

g.getCountry = () =>
  findAll("select, input")
    .map(getText)
    .concat([getPriceText()]) /*.map(t => (t+'').strip())*/;
...
g.set = (obj, path, value) =>
  setTo(get(obj, path.slice(0, -1)), path[path.length - 1], value);
...
g.setValues = (v) =>
  recurse(g.y, (item, path, root) => item(get(v, path) + ""));
