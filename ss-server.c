...
static lws_ss_state_return_t
myss_srv_tx(void* userobj, lws_ss_tx_ordinal_t ord, uint8_t* buf, size_t* len, int* flags) {
  myss_srv_t* m = (myss_srv_t*)userobj;
...
spam_sul_cb(struct lws_sorted_usec_list* sul) {
  ...
  lws_sul_schedule(lws_ss_get_context(m->ss), 0, &m->sul, spam_sul_cb, 100 * LWS_US_PER_MS);
}

static lws_ss_state_return_t
myss_ws_tx(void* userobj, lws_ss_tx_ordinal_t ord, uint8_t* buf, size_t* len, int* flags) {
  myss_srv_t* m = (myss_srv_t*)userobj;
...
myss_srv_state(void* userobj, void* sh, lws_ss_constate_t state, lws_ss_tx_ordinal_t ack) {
  ...
  switch(state) {
    ...
    case LWSSSCS_CREATING: lws_ss_request_tx(m->ss); break;
