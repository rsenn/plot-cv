...
module_string_to_array_buffer(JSContext* ctx,
  ...
  /* Get as C string, utf-8 encoded, store length in 'len' */
  str = JS_ToCStringLen(ctx, &len, argv[0]);

  /* create new ArrayBuffer from that */
  ret = JS_NewArrayBufferCopy(ctx, (const uint8_t*)str, len);
...
static const JSCFunctionListEntry module_funcs[] = {
    ...
    JS_CFUNC_DEF("stringToArrayBuffer", 1, module_string_to_array_buffer),
};
