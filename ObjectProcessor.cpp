...
// PURSUIT OF COLOR SILHOUETTE is a C++ program that allows user to play around with options that include object
// tracking and identification,  contouring, all with their own windows computer webcam
...
static void
getMaxClass(const Mat& probBlob, int* classId, double* classProb) {
...
track_and_identify(int argc, char** argv) {
  ...
  Mat frame, hsv, hue, mask, hist, histimg = Mat::zeros(400, 640, CV_8UC3),
...
bg_sub_contour(int argc, char** argv) {
  ...
  cout << "This feed lets the user adjust the lightness of the colour, saturation and hue using the provided trackbars."
       << endl;
