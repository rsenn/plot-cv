...
const outputPath = args[1] || basename(inputPath, extname(inputPath)) + '.circuitjs';
...
const keep = components.filter(c => c.kind !== 'wire-frag' && c.kind !== 'node' && c.kind !== 'unknown');

console.log(`contours: ${contours.length}  kept components: ${keep.length}`);
for(const c of keep) console.log(`  ${c.kind.padEnd(10)} bbox=${c.rect.x},${c.rect.y} ${c.rect.width}x${c.rect.height}`);
