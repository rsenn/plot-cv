...
async function testVoronoi(filename) {
  ...
  const factory = SVG.factory({ create: tag => ({ tagName: tag }), append_to: (elem, parent) => (parent.children = add(parent.children, elem)), setattr: (elem, name, value) => { if (!elem.attributes) elem.attributes = {};
