...
inline string
containerByName(const string& name) {
...
createWriter(
    const string& backend, const string& file_name, const string& codec, Size sz, unsigned fps) {
  if(backend == "gst-default") {
    cout << "Created GStreamer writer ( " << file_name << ", FPS=" << fps << ", Size=" << sz << ")"
         << endl;
    ...
    cout << "Created FFMpeg writer ( " << file_name << ", FPS=" << fps << ", Size=" << sz << " )"
         << endl;
    return makePtr<VideoWriter>(
...
main(int argc, char* argv[]) {
  ...
  cout << "Mode: " << mode << ", Backend: " << backend << ", File: " << file_name
       << ", Codec: " << codec << endl;
  ...
  if(tick.getCounter() == 0) {
    ...
    cout << tick.getCounter() << " frames in " << tick.getTimeSec() << " sec ~ " << res_fps
         << " FPS"
