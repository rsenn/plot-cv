...
main(int argc, char* argv[]) {
  ...
  while(true) {
    if(mode == "decode") {
      ...
      int limit = 100;
      while(!cap->grab() && --limit != 0) { cout << "Skipping empty input frame - " << limit << endl; }
  ...
  if(tick.getCounter() == 0) {
    ...
    cout << tick.getCounter() << " frames in " << tick.getTimeSec() << " sec ~ " << res_fps << " FPS"
