...
decomposeHomography(const string& img1Path,
  ...
  int solutions = cv::decomposeHomographyMat(homography, cameraMatrix, Rs_decomp, ts_decomp, normals_decomp);
  ...
  //! [decompose-homography-estimated-by-cv::findHomography]
  solutions = cv::decomposeHomographyMat(H, cameraMatrix, Rs_decomp, ts_decomp, normals_decomp);
  cout << "Decompose homography matrix estimated by cv::findHomography():" << endl << endl;
  for(int i = 0; i < solutions; i++) {
    double factor_d1 = 1.0 / d_inv1;
    cv::Mat rvec_decomp;
    cv::Rodrigues(Rs_decomp[i], rvec_decomp);
