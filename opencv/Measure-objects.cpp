...
int main()
{
	...
	// creating windows
	for (int i = 0; i < 4; i++)
		namedWindow(window_name[i], cv::WINDOW_AUTOSIZE);
	...
	{										// capturing and analizing 27 fps
		...
		if (i_cont1 >= 0)	//analise of binary1 (hsv split 1)
		{
			...
			s = out.str();
			putText(img, s, Point(10, 10), cv::FONT_HERSHEY_COMPLEX, 0.4,					//puting information about rectangles in img window
					Scalar(20, 40, 80), 1, 8);
		...
		if (i_cont2 >= 0)	//analise of binary2 (hsv split 2), same as 1st
		{
			...
			st = out1.str();
			putText(img, st, Point(10, 40), cv::FONT_HERSHEY_COMPLEX, 0.4,
					Scalar(20, 40, 80), 1, 8);
		...
		st2 = out3.str();
		putText(img, st2, Point(450, 40), cv::FONT_HERSHEY_COMPLEX, 0.4,			//puting dimensions in mm in img window
				Scalar(20, 40, 80), 1, 8);
