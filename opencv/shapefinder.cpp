...
bool detect_shape() {
  ...
  double high =
      threshold(gray, edges, 0, 255, cv::THRESH_BINARY | cv::THRESH_OTSU);
  double low = 0.5 * high;
  ...
  findContours(edges.clone(), contours, cv::RETR_EXTERNAL,
  ...
  for (int i = 0; i < contours.size(); i++) {
    ...
    if (approx.size() == 4) {
      ...
      if (isSquare && args["-s"] == "square" ||
        ...
        rectangle(output, Point(bottom_x, bottom_y), Point(top_x, top_y),
                  Scalar(rgb_color[2], rgb_color[1], rgb_color[0]), 3, 8, 0);
        found_shape = true;
      }
    } else {
      ...
      HoughCircles(gray, circles, cv::HOUGH_GRADIENT, 1, gray.rows / 8, 200, 100,
                   0, 0);
