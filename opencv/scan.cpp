/* DO NOT EDIT THIS FILE - cv::it is machine generated */
...
vector<cv::Point>
getPoints(cv::Mat image) {
  int width = image.size().width;
  int height = image.size().height;
  cv::Mat image_proc = image.clone();
  vector<vector<cv::Point>> squares;
  // cv::blur will enhance edge detection
  cv::Mat blurred(image_proc);
  cv::medianBlur(image_proc, blurred, 9);
  ...
  for(int c = 0; c < 3; c++) {
    ...
    for(int i = 0; i < squares.size(); i++) {
      double a = cv::contourArea(squares[i], false);
      if(a > largest_area) {
        largest_area = a;
    ...
    if(squares.size() > 0) {
      ...
      points.push_back(cv::Point(width, 0));
      points.push_back(cv::Point(0, height));
...
cv::Mat
scan(cv::Mat cv::img, jfloat x1, jfloat y1, jfloat x2, jfloat y2, jfloat x3, jfloat y3, jfloat x4, jfloat y4) {

  __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "Scaning scan() %f", x1);
  // define the destination image size:

  float w1 = sqrt(cv::pow(x4 - x3, 2) + cv::pow(x4 - x3, 2));
  float w2 = sqrt(cv::pow(x2 - x1, 2) + cv::pow(x2 - x1, 2));
  float h1 = sqrt(cv::pow(y2 - y4, 2) + cv::pow(y2 - y4, 2));
  float h2 = sqrt(cv::pow(y1 - y3, 2) + cv::pow(y1 - y3, 2));

  float maxWidth = (w1 < w2) ? w1 : w2;
  float maxHeight = (h1 < h2) ? h1 : h2;

  cv::Mat dst = cv::Mat::zeros(maxHeight, maxWidth, CV_8UC3);
  ...
  dst_pts.push_back(cv::Point(maxWidth - 1, 0));
  dst_pts.push_back(cv::Point(0, maxHeight - 1));
  dst_pts.push_back(cv::Point(maxWidth - 1, maxHeight - 1));
  ...
  cv::warpPerspective(cv::img, dst, transmtx, dst.size());

  return dst;
}

jobject
mat_to_bitmap(JNIEnv* env, cv::Mat& src, bool needPremultiplyAlpha, jobject bitmap_config) {
  ...
  try {
    ...
    CV_Assert(pixels);
    if(info.cv::format == ANDROID_BITMAP_FORMAT_RGBA_8888) {
      ...
      // info.cv::format == ANDROID_BITMAP_FORMAT_RGB_565
      cv::Mat tmp(info.height, info.width, CV_8UC2, pixels);
      if(src.type() == CV_8UC1) {
        cv::cvtColor(src, tmp, cv::COLOR_GRAY2BGR565);
      } else if(src.type() == CV_8UC3) {
        cv::cvtColor(src, tmp, CV_RGB2BGR565);
      } else if(src.type() == CV_8UC4) {
...
Java_vi_pdfscanner_main_ScannerEngine_getScannedBitmap(JNIEnv* env,
                                                       jobject thiz,
                                                       jobject bitmap,
                                                       jfloat x1,
                                                       jfloat y1,
  ...
  if((ret = AndroidBitmap_getInfo(env, bitmap, &info)) < 0) {
    __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "AndroidBitmap_getInfo() failed ! cv::error=%d", ret);
    return NULL;
  }

  if(info.cv::format != ANDROID_BITMAP_FORMAT_RGBA_8888) {
    __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "Bitmap cv::format is not RGBA_8888!");
    return NULL;
  }

  if((ret = AndroidBitmap_lockPixels(env, bitmap, &pixels)) < 0) {
    __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "AndroidBitmap_lockPixels() failed ! cv::error=%d", ret);
  }

  // init our output image
  cv::Mat mbgra(info.height, info.width, CV_8UC4, pixels);
  cv::Mat dst = scan(mbgra, x1, y1, x2, y2, x3, y3, x4, y4);
...
JNIEXPORT jobject JNICALL
Java_vi_pdfscanner_main_ScannerEngine_getMagicColorBitmap(JNIEnv* env, jobject thiz, jobject bitmap) {
  ...
  cv::Mat mbgra(info.height, info.width, CV_8UC4, pixels);
  cv::Mat dst = mbgra.clone();
  // init our output image
  // float alpha = 1.9;
  // float beta = -80;
  // dst.convertTo(dst, -1, alpha, beta);
  cv::cvtColor(mbgra, dst, cv::COLOR_BGR2GRAY);
  cv::adaptiveThreshold(dst, dst, 255, cv::ADAPTIVE_THRESH_MEAN_C, cv::THRESH_BINARY, 5, 4);
...
JNIEXPORT jobject JNICALL
Java_vi_pdfscanner_main_ScannerEngine_getBWBitmap(JNIEnv* env, jobject thiz, jobject bitmap) {
  ...
  cv::threshold(dst, dst, 0, 255, cv::THRESH_BINARY | THRESH_OTSU);
...
JNIEXPORT jobject JNICALL
Java_vi_pdfscanner_main_ScannerEngine_getGrayBitmap(JNIEnv* env, jobject thiz, jobject bitmap) {
...
Java_vi_pdfscanner_main_ScannerEngine_getPoints(JNIEnv* env, jobject thiz, jobject bitmap) {
  ...
  // init our output image
  cv::Mat mbgra(info.height, info.width, CV_8UC4, pixels);
  vector<cv::Point> img_pts = getPoints(mbgra);
