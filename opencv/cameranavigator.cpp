...
int32_t
FindLineCenter(raspicam::RaspiCam_Cv& Camera, int32_t nTick, int32_t& nCenterX, int32_t& nCenterY) {
  cv::Mat image;
...

  {
...
#if TESTROI
    {
      ...
      try {
        // cv::drawContours(roiImg, contours, s, cv::Scalar(255, 255, 255), 2, 8, hierarchy, 0,
        // cv::Point() );
...
int32_t
FindOffset(int32_t nCenterX, int32_t nCenterY, float& fXOffset, float& fYOffset) {
  int32_t nRet = 0;
...
int32_t
OffsetNavigator(ros::ServiceClient& client, float fXOffset) {
  int32_t nRet = 0;
