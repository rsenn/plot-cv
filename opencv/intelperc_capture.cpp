...
main(int argc, char* argv[]) {
  ...
  for(;; frame++) {
    ...
    if(g_printTiming) {
      ...
      cout << "Image frame: " << capture.get(CV_CAP_INTELPERC_IMAGE_GENERATOR | CV_CAP_PROP_POS_MSEC) << ", Depth(IR) frame: " << capture.get(CV_CAP_INTELPERC_DEPTH_GENERATOR | CV_CAP_PROP_POS_MSEC)
           << endl;
    }
    if(waitKey(30) >= 0)
      break;
