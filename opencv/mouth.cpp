...
main(int argc, char** argv) {
  ...
  // Mark end-points
  cv::circle(image_contour, cv::Point(min_x, min_y), 3.0, cv::Scalar(0, 0, 255), -1, 8);
  cv::circle(image_contour, cv::Point(max_x, max_y), 3.0, cv::Scalar(0, 0, 255), -1, 8);
  cv::line(image_contour, cv::Point(min_x, min_y), cv::Point(max_x, max_y), cv::Scalar(0, 0, 255), 1, 8);

  // Mark mid-points
  for(int i = 0; i < mid_y_values.size(); ++i) {
    cv::circle(image_contour, cv::Point(closest_mid_x, mid_y_values[i]), 3.0, cv::Scalar(0, 0, 255), -1, 8);
    cv::line(image_contour, cv::Point(closest_mid_x, mid_y_values[i]), cv::Point(min_x, min_y), cv::Scalar(0, 0, 255), 1, 8);
  ...
  // cv::imshow("Pseudo-Hue", pseudo_hue_plane);
  // cv::imshow("Pseudo-Hue-Binary", pseudo_hue_bin);
   ...
   *
   * cv::imshow("Chrominance-Plane", chrominance_plane);
   * cv::imshow("Modified-Chrominance-Plane", modified_chrominance_plane);
   *
   */
...
extractMouthROI(cv::Mat_<cv::Vec3b> face_image) {
  ...
  return face_image(cv::Rect(mouth_x, mouth_y, mouth_cols, mouth_rows));
}
...
equalizeImage(cv::Mat_<cv::Vec3b> image_BGR) {
  ...
  cv::merge(channels, image_eq);
  cv::cvtColor(image_eq, image_eq, CV_YCrCb2BGR);
...
transformLUX(cv::Mat_<cv::Vec3b> image_BGR) {
  ...
  for(int i = 0; i < image_BGR.rows; ++i) {
    for(int j = 0; j < image_BGR.cols; ++j) {
      ...
      L = (cv::pow(R + 1, 0.3) * cv::pow(G + 1, 0.6) * cv::pow(B + 1, 0.1)) - 1;
      L_int = round(L);
