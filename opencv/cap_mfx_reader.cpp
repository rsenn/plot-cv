...
double
VideoCapture_IntelMFX::getProperty(int) const {
  MSG(cerr << "MFX: getProperty() is not implemented" << endl);
  return 0;
}

bool
VideoCapture_IntelMFX::setProperty(int, double) {
...
bool
VideoCapture_IntelMFX::retrieveFrame(int, OutputArray out) {
  if(!outSurface) {
...
bool
VideoCapture_IntelMFX::isOpened() const {
  return good;
}

int
VideoCapture_IntelMFX::getCaptureDomain() {
...
cv::create_MFX_capture(const std::string& filename) {
