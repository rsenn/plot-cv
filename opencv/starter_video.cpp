 ...
 *      Author: Kevin Hughes
 *
 * A starter sample for using OpenCV cv::VideoCapture with capture devices, video files or image
...
process(cv::VideoCapture& capture) {
  ...
  cv::namedWindow(window_name, cv::WINDOW_NORMAL); // resizable window;
  cv::Mat frame;
