...
public:
  ...
  create(void) {
    ...
    r = m_pD3D9Ex->CreateDeviceEx(
        D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, m_hWnd, flags, &d3dpp, NULL, &m_pD3D9DevEx);
    if(FAILED(r)) {
      return -1;
    }
  ...
  void
  print_info(LPDIRECT3DSURFACE9 pSurface, int mode, float time, cv::String oclDevName) {
    HDC hDC;
