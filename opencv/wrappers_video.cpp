...
enum UserMemoryMode { COPY, USER_MEM, MAP };
...
ivx::Graph
createProcessingGraph(ivx::Image& inputImage, ivx::Image& outputImage) {
  using namespace ivx;
  ...
  // Intermediate images
  Image yuv = Image::createVirtual(graph, 0, 0, VX_DF_IMAGE_YUV4), gray = Image::createVirtual(graph), smoothed = Image::createVirtual(graph), cannied = Image::createVirtual(graph),
        halfImg = Image::create(context, width, height, VX_DF_IMAGE_U8), halfCanny = Image::create(context, width, height, VX_DF_IMAGE_U8);
...
main(int argc, char* argv[]) {
  const std::string keys = "{help h usage ? | | }"
  ...
  parser.about("OpenVX interoperability sample demonstrating OpenVX wrappers usage."
               "The application opens a video and processes it with OpenVX graph while outputting "
               "result in a window");
  if(parser.has("help")) {
    parser.printMessage();
    return 0;
