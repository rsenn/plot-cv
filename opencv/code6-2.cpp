...
make_contours_closed(std::vector<std::vector<cv::Point>> contours) {
  for(int i = 0; i < contours.size(); i++) {
    ...
    cv::approxPolyDP(contours[i], cc, 0.1, true);
    contours[i] = cc;
  }
}
...
int
smallest_contour(cv::Point p, std::vector<std::vector<cv::Point>> contours, std::vector<cv::Vec4i> heirarchy) {
...
int
main() {
  cv::Mat img = cv::imread("rects.png");
  cv::Mat edges;
  ...
  cv::drawContours(img_all_contours, contours, -1, cv::Scalar(0, 255, 0));
