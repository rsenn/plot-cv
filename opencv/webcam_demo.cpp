...
template<class T> class Parallel_OCR : public cv::ParallelLoopBody {
private:
...
public:
  ...
  operator()(const cv::Range& r) const CV_OVERRIDE {
    for(int c = r.start; c < r.end; c++) {
      ocrs[c % ocrs.size()]->run(
          detections[c], outputs[c], &boxes[c], &words[c], &confidences[c], OCR_LEVEL_WORD);
    }
  }
...
main(int argc, char* argv[]) {
  ...
  cout << "A demo program of End-to-end Scene Text Detection and Recognition using webcam or video."
       << endl
       << endl;
  cout << "  Keys:  " << endl;
  ...
  for(int i = 0; i < 2; i++) {
    Ptr<ERFilter> er_filter1 = createERFilterNM1(
        loadClassifierNM1("trained_classifierNM1.xml"), 8, 0.00015f, 0.13f, 0.2f, true, 0.1f);
    Ptr<ERFilter> er_filter2 =
        createERFilterNM2(loadClassifierNM2("trained_classifierNM2.xml"), 0.5);
    er_filters1.push_back(er_filter1);
    er_filters2.push_back(er_filter2);
  }

  // Initialize OCR engine (we initialize 10 instances in order to work several recognitions in
  // parallel)
  ...
  for(int o = 0; o < num_ocrs; o++) {
    decoders.push_back(OCRHMMDecoder::create(
        loadOCRHMMClassifierNM("OCRHMM_knn_model_data.xml.gz"), voc, transition_p, emission_p));
  }
  cout << " Done!" << endl;
  ...
  while(true) {
    ...
    switch(REGION_TYPE) {
      ...
      case 1: // MSER
        ...
        Ptr<MSER> mser = MSER::create(21,
                                      (int)(0.00002 * gray.cols * gray.rows),
                                      (int)(0.05 * gray.cols * gray.rows),
                                      1,
                                      0.7);
    ...
    switch(GROUPING_ALGORITHM) {
      case 0: // exhaustive_search
        erGrouping(
            frame, channels, regions, nm_region_groups, nm_boxes, ERGROUPING_ORIENTATION_HORIZ);
        break;
    ...
    copyMakeBorder(
        frame, out_img, 0, bottom_bar_height, 0, 0, BORDER_CONSTANT, Scalar(150, 150, 150));
    ...
    for(int i = 0; i < (int)detections.size(); i = i + (int)num_ocrs) {
      ...
      switch(RECOGNITION) {
        case 0: // Tesseract
          parallel_for_(
              r, Parallel_OCR<OCRTesseract>(detections, outputs, boxes, words, confidences, ocrs));
          break;
        case 1: // NM_chain_features + KNN
          parallel_for_(r,
                        Parallel_OCR<OCRHMMDecoder>(
                            detections, outputs, boxes, words, confidences, decoders));
          break;
      }
    }
    ...
    for(int i = 0; i < (int)detections.size(); i++) {
      ...
      for(int j = 0; j < (int)boxes[i].size(); j++) {
        ...
        Size word_size = getTextSize(
            words[i][j], FONT_HERSHEY_SIMPLEX, (double)scale_font, (int)(3 * scale_font), NULL);
        rectangle(out_img,
  ...
  for(int r = 0; r < (int)group.size(); r++) {
    ...
    if(er.parent != NULL) { // deprecate the root region
      ...
      floodFill(channels[group[r][0]],
                segmentation,
                cv::Point(er.pixel % channels[group[r][0]].cols,
