...
public:
  Parallel_extractCSER(std::vector<cv::Mat>& _channels, std::vector<std::vector<ERStat>>& _regions, std::vector<Ptr<ERFilter>> _er_filter1, std::vector<Ptr<ERFilter>> _er_filter2)
  ...
  while(true) {
    ...
    for(int i = 0; i < (int)detections.size(); i++) {
      ...
      for(int j = 0; j < (int)boxes[i].size(); j++) {
        ...
        if((words[i][j].size() < 2) || (confidences[i][j] < min_confidence1) || ((words[i][j].size() == 2) && (words[i][j][0] == words[i][j][1])) ||
