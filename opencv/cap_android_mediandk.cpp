...
static inline void
deleter_AMediaExtractor(AMediaExtractor* extractor) {
  AMediaExtractor_delete(extractor);
}

static inline void
deleter_AMediaCodec(AMediaCodec* codec) {
...
public:
  ...
  bool
  retrieveFrame(int, OutputArray out) CV_OVERRIDE {
    if(buffer.empty()) {
      return false;
    }
  ...
  bool
  setProperty(int /* property_id */, double /* value */) CV_OVERRIDE {
    return false;
  }

  bool
  initCapture(const char* filename) {
...
Ptr<IVideoCapture>
cv::createAndroidCapture_file(const std::string& filename) {
