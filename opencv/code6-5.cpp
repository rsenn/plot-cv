// Program to find the largest ellipse using RANSAC and show bounding rectangles and circle around
...
ellipseFinder::is_good_ellipse(cv::Mat Q) {
  ...
  float thresh = 0.09, num = 2 * (a * f * f + c * d * d + g * b * b - 2 * b * d * f - a * c * g), den1 = (b * b - a * c) * (sqrt((a - c) * (a - c) + 4 * b * b) - (a + c)), den2 = (b * b - a * c) * (-sqrt((a - c) * (a - c) + 4 * b * b) - (a + c)), a_len = sqrt(num / den1), b_len = sqrt(num / den2),
        major_axis = max(a_len, b_len), minor_axis = min(a_len, b_len);
...
ellipseFinder::ellipse_contour(cv::Mat Q) {
  ...
  float num = 2 * (a * f * f + c * d * d + g * b * b - 2 * b * d * f - a * c * g), den1 = (b * b - a * c) * (sqrt((a - c) * (a - c) + 4 * b * b) - (a + c)), den2 = (b * b - a * c) * (-sqrt((a - c) * (a - c) + 4 * b * b) - (a + c)), a_len = sqrt(num / den1), b_len = sqrt(num / den2), major_axis = max(a_len, b_len),
...
ellipseFinder::draw_ellipse(cv::Mat Q) {
  ...
  cv::Point2f center;
  float radius;
  minEnclosingCircle(ellipse, center, radius);
  ...
  for(int i = 0; i < 4; i++) line(img_show, vertices[i], vertices[(i + 1) % 4], Scalar(0, 255, 0)); // green
