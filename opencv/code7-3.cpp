...
objectCounter::get_markers() {
  ...
  cv::Mat strel = getStructuringElement(MORPH_ELLIPSE, Size(9, 9));
  cv::Mat im_d;
  dilate(im_e, im_d, strel);
  // imshow("im_d", im_d);
  ...
  cv::Mat th_a;
  adaptiveThreshold(im_oc, th_a, 255, ADAPTIVE_THRESH_MEAN_C, THRESH_BINARY, 105, 0);
  // imshow("th_a", th_a);
  ...
  findContours(th_e, c, heirarchy, cv::RETR_CCOMP, cv::CHAIN_APPROX_NONE);
...
objectCounter::count_objects() {
  ...
  cv::Mat imgGray;
  cvtColor(gray, imgGray, cv::COLOR_GRAY2BGR);
