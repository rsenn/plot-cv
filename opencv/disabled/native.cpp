...
static cv::Size
calc_optimal_camera_resolution(const char* supported, int width, int height) {
  int frame_width = 0;
  int frame_height = 0;
  ...
  do {
    ...
    prev_idx = idx;
    while((supported[idx] != '\0') && (supported[idx] != ',')) idx++;
...
static void
engine_draw_frame(Engine* engine, const cv::Mat& frame) {
...
static void
engine_handle_cmd(android_app* app, int32_t cmd) {
  ...
  switch(cmd) {
    case APP_CMD_INIT_WINDOW:
      if(app->window != NULL) {
        ...
        u.prop = engine->capture->get(cv::CAP_PROP_SUPPORTED_PREVIEW_SIZES_STRING);
