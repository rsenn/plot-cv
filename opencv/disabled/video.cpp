...
main(int argc, char** argv) {
  ...
  while(/*(char(waitKey(1)) != 27) ||*/ (ros::ok())) {
    ...
    cv::Mat element = getStructuringElement(MORPH_RECT, Size(10, 10)); // 15,15
    ...
    for(int i = 0; i < contours.size(); i++) {
      ...
      if(radius[i] > 40) {
        circle(frame, center[i], radius[i], Scalar(255, 0, 0), 3, cv::LINE_AA, 0);
        circle(frame, center[i], 2, Scalar(0, 0, 255), 3, cv::LINE_AA, 0);
    ...
    while(it != trac.end()) {
      ...
      if(p.x == 0 || p2.x == 0) {
        ...
        line(frame, *it, *(it + 1), Scalar(0, 255, 0), thickness);
      else {
        trac.pop_back();
        continue;
      }
