...
class Thread {
  ...
  static DWORD WINAPI
  WinThreadFunction(LPVOID lpParam) {
...
void
StereoMultiGpuThread::launchGpuStereoAlg(void* userData) {
...
main(int argc, char** argv) {
  ...
  for(int i = 0;; ++i) {
    ...
    cout << "| " << setw(5) << i << " | " << setw(8) << setprecision(1) << fixed << gpu0Time << " | " << setw(8) << setprecision(1) << fixed << gpu1Time << " | " << setw(15) << setprecision(1) << fixed << multiThreadTime << " | " << setw(15) << setprecision(1) << fixed << multiStreamTime << " |" << endl;
