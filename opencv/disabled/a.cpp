...
main() {
  ...
  while(charCheckForEscKey != 27 && cap.isOpened()) {
    ...
    for(int i = 0; i < v3fCircles.size(); i++) {
      ...
      line(matOriginal, Point(160, 240), Point(x_center, y_center), Scalar(0, 255, 0), 2, cv::LINE_AA);
    ...
    line(matOriginal, Point(160, 240), Point(160, 0), Scalar(0, 0, 0), 1, cv::LINE_AA);
