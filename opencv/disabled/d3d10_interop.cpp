...
public:
  ...
  create(void) {
    ...
    if(cv::ocl::haveOpenCL()) {
      m_oclCtx = cv::directx::cv::ocl::initializeContextFromD3D10Device(m_pD3D10Dev);
    }
      ...
      switch(mode) {
        case MODE_CPU: {
          ...
          if(m_demo_processing) {
            // cv::blur D3D10 surface with OpenCV on CPU
        ...
        case MODE_GPU_RGBA: {
          ...
          if(m_demo_processing) {
            // cv::blur D3D10 surface with OpenCV on GPU with OpenCL
