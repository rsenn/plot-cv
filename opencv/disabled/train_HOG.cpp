...
void load_images(const string& prefix, const string& filename, vector<cv::Mat>& img_lst);
void sample_neg(const vector<cv::Mat>& full_neg_lst, vector<Mat>& neg_lst, const cv::Size& size);
cv::Mat get_hogdescriptor_visu(const cv::Mat& color_origImg, vector<float>& descriptorValues, const cv::Size& size);
void compute_hog(const vector<cv::Mat>& img_lst, vector<Mat>& gradient_lst, const cv::Size& size);
void train_svm(const vector<cv::Mat>& gradient_lst, const vector<int>& labels);
void draw_locations(cv::Mat& img, const vector<cv::Rect>& locations, const cv::Scalar& color);
void test_it(const cv::Size& size);

void
get_svm_detector(const Ptr<SVM>& svm, vector<float>& hog_detector) {
 ...
 * TrainData is a matrix of size (#cv::samples x max(#cols,#rows) per cv::samples), in 32FC1.
 * Transposition of cv::samples are made if needed.
 */
void
convert_to_ml(const std::vector<cv::Mat>& train_samples, cv::Mat& trainData) {
  //--Convert data
  const int rows = (int)train_samples.size();
  const int cols = (int)std::max(train_samples[0].cols, train_samples[0].rows);
  cv::Mat tmp(1, cols, CV_32FC1); //< used for transposition if needed
  trainData = cv::Mat(rows, cols, CV_32FC1);
  vector<cv::Mat>::const_iterator itr = train_samples.begin();
  vector<cv::Mat>::const_iterator end = train_samples.end();
...
void
load_images(const string& prefix, const string& filename, vector<cv::Mat>& img_lst) {
  string cv::line;
  ifstream file;
  ...
  while(!end_of_parsing) {
    getline(file, cv::line);
    if(cv::line.empty()) { // no more file to read
      end_of_parsing = true;
      break;
    }
...
void
sample_neg(const vector<cv::Mat>& full_neg_lst, vector<Mat>& neg_lst, const cv::Size& size) {
  cv::Rect box;
  ...
  vector<cv::Mat>::const_iterator img = full_neg_lst.begin();
  vector<cv::Mat>::const_iterator end = full_neg_lst.end();
  for(; img != end; ++img) {
...
Mat
get_hogdescriptor_visu(const cv::Mat& color_origImg, vector<float>& descriptorValues, const cv::Size& size) {
  ...
  for(int y = 0; y < cells_in_y_dir; y++) {
    ...
    for(int x = 0; x < cells_in_x_dir; x++) {
      ...
      for(int bin = 0; bin < gradientBinSize; bin++) gradientStrengths[y][x][bin] = 0.0;
    }
  }
  ...
  for(int blockx = 0; blockx < blocks_in_x_dir; blockx++) {
    for(int blocky = 0; blocky < blocks_in_y_dir; blocky++) {
      ...
      for(int cellNr = 0; cellNr < 4; cellNr++) {
        ...
        // note: overlapping blocks lead to multiple updates of this cv::sum!
...
void
compute_hog(const vector<cv::Mat>& img_lst, vector<Mat>& gradient_lst, const cv::Size& size) {
...
void
train_svm(const vector<cv::Mat>& gradient_lst, const vector<int>& labels) {
...
void
draw_locations(cv::Mat& img, const vector<cv::Rect>& locations, const cv::Scalar& color) {
  if(!locations.empty()) {
    vector<cv::Rect>::const_iterator loc = locations.begin();
    vector<cv::Rect>::const_iterator end = locations.end();
    for(; loc != end; ++loc) { cv::rectangle(img, *loc, color, 2); }
  }
}

void
test_it(const cv::Size& size) {
  char key = 27;
...
main(int argc, char** argv) {
  ...
  vector<cv::Mat> pos_lst;
  vector<cv::Mat> full_neg_lst;
  vector<cv::Mat> neg_lst;
  vector<cv::Mat> gradient_lst;
  vector<int> labels;
  ...
  sample_neg(full_neg_lst, neg_lst, cv::Size(96, 160));
  labels.insert(labels.end(), neg_lst.size(), -1);
  CV_Assert(old < labels.size());

  compute_hog(pos_lst, gradient_lst, cv::Size(96, 160));
  compute_hog(neg_lst, gradient_lst, cv::Size(96, 160));

  train_svm(gradient_lst, labels);

  test_it(cv::Size(96, 160)); // change with your parameters
