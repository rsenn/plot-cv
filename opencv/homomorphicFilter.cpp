...
main(int argc, char** argv) {
  if(argc == 2 && (!strcmp(argv[1], "-h") || !strcmp(argv[1], "--help"))) {
    std::cerr << "This is a program for performing homomorphic filtering on an image." << std::endl;
...
void
fft(const cv::Mat& src, cv::Mat& dst) {
  ...
  cv::copyMakeBorder(
      logimg, padded, 0, m - logimg.rows, 0, n - logimg.cols, cv::BORDER_CONSTANT, cv::Scalar::all(0));
...
cv::Mat
butterworth(const cv::Mat& img, int d0, int n, int high, int low) {
