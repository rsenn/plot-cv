...
public:
  ...
  WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
    switch(message) {
      ...
      case WM_CLOSE: cleanup(); return 0;
    ...
    switch(e.type) {
      ...
      case KeyPress:
        switch(keycode_to_keysym(e.xkey.keycode)) {
          case XK_space: m_demo_processing = !m_demo_processing; break;
          case XK_1: set_mode(MODE_CPU); break;
          case XK_2: set_mode(MODE_GPU); break;
          case XK_9: toggle_buffer(); break;
          case XK_Escape:
            m_end_loop = true;
            cleanup();
            break;
  ...
  init() {
    ...
    m_oclDevName = cv::ocl::useOpenCL() ? cv::ocl::Context::getDefault().device(0).name()
  ...
  void
  print_info(MODE mode, float time, cv::String& oclDevName) {
#if defined(WIN32) || defined(_WIN32)
    HDC hDC = m_hDC;
...
protected:
  void
  processFrameCPU(cv::ogl::Texture2D& texture, cv::ogl::Buffer& buffer, bool do_buffer) {
    cv::Mat m(m_height, m_width, CV_8UC4);
  ...
  void
  processFrameGPU(cv::ogl::Texture2D& texture, cv::ogl::Buffer& buffer, bool do_buffer) {
    cv::UMat u;
...
#if defined(__linux__)
  KeySym
  keycode_to_keysym(unsigned keycode) {
...
static const char* keys = {"{c camera | true  | use camera or not}"
