...
cv::Vec4b
getColorSubpix(const cv::Mat& img, cv::Point2f val) {
  cv::Mat patch;
...
static void
colorize(float minval, float maxval, const cv::Mat& in_, const cv::Mat& palette_, cv::Mat& res) {
...
main(int argc, char* argv[]) {
  ...
  cv::Vec4f palette_jet_colors[] = {cv::Vec4f(0.7, 0.0, 0.1, 1.0),
