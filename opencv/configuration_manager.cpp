...
Configuration_Manager::init_scheduler() {
  ...
  Scheduler* new_scheduler =
      new Scheduler(camera_index, event_save_dir, cascade_path, primary_cascade_name, secondary_cascade_name, camera_uninit);
    ...
    switch(choice) {
      case('1'):
        ...
        if(!(this->get_scheduler_list_size() > 0)) {
          ...
          if(selected_scheduler == NULL) {
            cout << "Configuration_Manager::select_camera_feed() failed to access scheduler_" << scheduler_index_selection
                 << endl;
            return -1;
          } else {
