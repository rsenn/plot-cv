...
Configuration_Manager::init_scheduler() {
  ...
  if(!camera_uninit.open(camera_index)) {
    cout << "Error: Configuration_Manager::init_scheduler() failed to open camera at camera_index="
         << camera_index << ".\n";
  ...
  Scheduler* new_scheduler = new Scheduler(camera_index,
                                           event_save_dir,
                                           cascade_path,
                                           primary_cascade_name,
                                           secondary_cascade_name,
                                           camera_uninit);
...
Configuration_Manager::display_all_feeds() {
  ...
  while(1) { // Read frames and update windows until exit signal
    for(unsigned int i = 0; i < cameras.size();
        i++) { // Read frame from each camera and update it's window
