          ...
          if(value.isReal())
            ...
            for(size_t i = 0; i < value.size(); ++i) {
              ...
              if(v.isInt())
                ...
                CV_Error(cv::Error::StsNotImplemented, "Unexpected value cv::format");
            }
          } else
            CV_Error(cv::Error::StsNotImplemented, "Unexpected field cv::format");
        }
      }
    }
  }
...
genPreprocArguments(const std::string& modelName, const std::string& zooFile) {
  return genArgument("model",
         ...
         genArgument("config",
                     ...
                     'c') +
         genArgument("cv::mean",
                     "Preprocess input image by subtracting cv::mean values. Mean values should be in BGR order and "
                     "delimited by spaces.",
                     modelName,
                     zooFile) +
