...
const CONFIG = {
  ...
  confThreshold: 0.4, // Mindest-Konfidenz
...
const CATEGORY_MAP = {
  ...
  banana: 'Lebensmittel',
  apple: 'Lebensmittel',
  sandwich: 'Lebensmittel',
  orange: 'Lebensmittel',
  broccoli: 'Lebensmittel',
  carrot: 'Lebensmittel',
  'hot dog': 'Lebensmittel',
  pizza: 'Lebensmittel',
  donut: 'Lebensmittel',
  cake: 'Lebensmittel',
  'wine glass': 'Lebensmittel',
  cup: 'Lebensmittel',
  fork: 'Lebensmittel',
  knife: 'Lebensmittel',
  spoon: 'Lebensmittel',
...
/** Kopiert eine Datei (Fallback für rename bei Datei-System-Grenzen) */
function copyFile(src, dst) {
  const d = readFileSync(src, null);
  writeFileSync(dst, d);
}
...
function main() {
  ...
  const [entries, dirErr] = readdir(inputDir);
  if(dirErr !== 0) {
    log('err', `Kann Verzeichnis nicht lesen: ${inputDir}`);
    exit(1);
  }
  ...
  // Zusammenfassung
  err.puts('\n────────────────────────────────────────\n');
  err.puts('📊 Zusammenfassung\n');
  err.puts('────────────────────────────────────────\n');
  for(const [cat, count] of Object.entries(stats)) {
    if(count > 0) err.puts(`   ${cat.padEnd(20)} ${count} Bild(er)\n`);
  }
  err.puts(`   ${'Fehler'.padEnd(20)} ${errors}\n`);
  err.puts('────────────────────────────────────────\n');
  err.puts(`✅ Fertig! Ergebnisse in: ${outputDir}\n`);
