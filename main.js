...
function EagleMaps(project) {
  ...
  let eagle2dom = [
    ...Element.findAll('*[data-path]', project.object)
  ].map((e) => [transformPath(e.getAttribute('data-path')), e]);
  console.debug('eagle2dom:', eagle2dom);

  eagle2dom = eagle2dom
    .map(([p, e]) => [
      Util.tryCatch(
        () => project.doc.lookup(p),
        (ret) => ret,
        () =>
...
window.dom2eagle=eagle2dom.map(([k, v]) => [v, k]);*/
  ...
  maps.eagle2dom = Util.mapFunction(
    new WeakMap(
  ...
  maps.dom2eagle = Util.mapFunction(
    new WeakMap(eagle2dom.map(([k, v]) => [v, k]))
  );

  const [path2component, component2path] = project.renderer.maps.map(
    Util.mapFunction
  );
  const {
    path2obj,
...
async function LoadDocument(project, parentElem) {
  ...
  if (/*doc.type != 'lbr'*/ true) {
    ...
    let usedLayers = [...doc.layers.list].filter(
      (layer) => layer.elements.size > 0
    );
...
const AppMain = (window.onload = async () => {
    ...
    for (url of urls) {
      ...
      if (files) {
        ...
        let svgs = list.reduce((acc, file) => {
          ...
          file.svg = `${EagleDocument.baseOf(file.name)}.${EagleDocument.typeOf(
            file.name
          )}.svg`;
  ...
  (async function () {
    while (true) {
      await CreateWebSocket(null, null, (ws) => (window.socket = ws)).catch(
        console.error
      );
  ...
  React.render(
    h(SlotProvider, {}, [
      h(
        Panel,
        { className: classNames('buttons', 'no-select'), tag: 'header' },
        [
          h(Button, {
                      ...
                      if (side != 'drill') {
                        ...
                        if (gc.svg) {
                          ...
                          if (side == 'outline') {
                            ...
                            bbox = new Rect(new BBox().update(points)).round(
                              0.001
                            );
  ...
  false &&
    moveHandler.subscribe(function MoveEvent(event, prevEvent) {
      ...
      let zIndex = Util.find(
        Util.map(WalkUp(event.target), (e) =>
          e.style.getPropertyValue('z-index')
        ),
        (z) => /^[0-9]/.test(z)
      );
      ...
      event.layers = new Map(
        event.elements.map((e) => [
          e,
          Element.walkUp(e, (e) => {
            if (e.hasAttribute('data-layer'))
              throw e.getAttribute('data-layer');
          })
        ])
      );
          ...
          let transforms =
            Element.walkUp(e, (p, d, set, stop) =>
              ...
              p.parentElement.isSameNode(p.ownerSVGElement)
                ? stop()
                : p.hasAttribute('transform') &&
                  set(p.getAttribute('transform'))
            ) || [];
