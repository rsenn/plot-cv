 ...
 */
void
main_test_input(unsigned int somethingStupid) {
  PmStream* midi;
  ...
  /* open input device */
  Pm_OpenInput(&midi, i, DRIVER_INFO, INPUT_BUFFER_SIZE, TIME_PROC, TIME_INFO);
...
void
main_test_output(int isochronous_test) {
  PmStream* midi;
  int32_t off_time;
...
#define PROGRAM 0
  ...
  if(isochronous_test) { // play 4 notes per sec for 20s
    ...
    WAIT_FOR_ENTER
    Pm_WriteShort(midi, Pt_Time(), Pm_Message(0x90, 60, 100));
    printf("ready to note-off (short form)... (type ENTER):");
    WAIT_FOR_ENTER
    Pm_WriteShort(midi, Pt_Time(), Pm_Message(0x90, 60, 0));
...
main_test_both() {
  ...
  Pm_OpenOutput(&midiOut, out, DRIVER_INFO, OUTPUT_BUFFER_SIZE, TIME_PROC, TIME_INFO, latency);
  printf("Midi Output opened with %ld ms latency.\n", (long)latency);
  /* open input device */
  Pm_OpenInput(&midi, in, DRIVER_INFO, INPUT_BUFFER_SIZE, TIME_PROC, TIME_INFO);
  printf("Midi Input opened. Reading %d Midi messages...\n", num);
...
main_test_stream() {
  ...
  /* open output device */
  Pm_OpenOutput(&midi, i, DRIVER_INFO, OUTPUT_BUFFER_SIZE, TIME_PROC, TIME_INFO, latency);
  printf("Midi Output opened with %ld ms latency.\n", (long)latency);
  ...
  while(n != 1) {
    ...
    switch(i) {
      ...
      case 3:
        ...
        somethingStupid = 2;
        break;
      case 4: test_output = 1; break;
      case 5: test_both = 1; break;
      case 6: stream_test = 1; break;
      case 7:
        test_output = 1;
        isochronous_test = 1;
        break;
      default: printf("got %d (invalid input)\n", n); break;
    }
  }
