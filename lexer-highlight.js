...
function classifyJs(tokens, i) {
  ...
  switch (tok.type) {
    ...
    case 'nestedEnd':
      return null;
    case 'punctuator':
      return JS_PLAIN_PUNCT.has(tok.lexeme) ? null : 'op';
    case 'identifier': {
      let j = i + 1;
      while(tokens[j] && tokens[j].type === 'whitespace') j++;
...
const CMAKE_KEYWORD = new Set(['if', 'elseif', 'else', 'endif', 'foreach', 'endforeach', 'while', 'endwhile', 'function', 'endfunction', 'macro', 'endmacro', 'break', 'continue', 'return']);
...
function classifyCMake(tokens, i) {
  ...
  switch (tok.type) {
    ...
    case 'unquotedArgument':
      return CMAKE_ATOM.has(tok.lexeme) ? 'atom' : null;
    default:
...
const SH_OP = new Set(['dsemi', 'semi', 'and_if', 'backgnd', 'or_if', 'pipe', 'bq', 'dlessdash', 'dless', 'lessand', 'lessgreat', 'less', 'dgreat', 'greatand', 'clobber', 'great']);
...
function classifyMake(tokens, i) {
  ...
  switch (tok.type) {
    ...
    case 'defineBackslash':
    case 'recipePrefix':
    case 'recipeBackslash':
      return null;
    default:
...
function classifyIni(tokens, i) {
  ...
  switch (tok.type) {
    ...
    case 'equals':
      return 'op';
    default:
      return null; // text, newline, whitespace: plain
  }
}
...
function classifyCsv(tokens, i) {
  ...
  switch (tok.type) {
    case 'separator':
      return 'op';
    case 'field':
      return tok.lexeme.startsWith('"') ? 'str' : null;
    default:
      return null; // nl: plain
  }
}
