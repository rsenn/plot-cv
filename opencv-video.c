//  g++ -Wl,-rpath=/opt/opencv-4.5.5-x86_64/lib $(pkgcfg --cflags  opencv4 |explode ' ')  -o opencv-video{,.c} -g -Os -w
//  -lav{format,codec,util} -lswresample  -lswscale $(pkgcfg --libs opencv4|explode ' ')
...
// The header file that needs to be introduced for the image conversion structure in the new version
...
main(int argc, char* argv[]) {
  ...
  // Traverse each stream, find the first video stream, and record the encoding information of the stream
  ...
  if((audioindex = av_find_best_stream(videoFormat, AVMEDIA_TYPE_AUDIO, -1, -1, &audioCodec, -1)) < 0) {
      ...
      if(packet->stream_index == videoindex) {
        ...
        /*  vector<Rect> faces(0);
          ...
          cascade.detectMultiScale(gray,
                                   ...
                                   2,
                                   2,
                                   cv::CASCADE_FIND_BIGGEST_OBJECT | cv::CASCADE_DO_ROUGH_SEARCH | cv::CASCADE_SCALE_IMAGE,
                                   cv::Size(50, 50));
          time(&t);
          printf("after %s\n", ctime(&t));
          Mat face;
