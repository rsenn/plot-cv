...
help(std::string errorMessage) {
  ...
  std::cout << "\nProgram call procedure : retinaDemo [processing mode] [Optional : media target] [Optional LAST "
...
main(int argc, char* argv[]) {
  ...
  std::cout << "* Benoit A., Caplier A., Durette B., Herault, J., \"USING HUMAN VISUAL SYSTEM MODELING FOR "
               ...
               "(2010), pp. 758-773, DOI: http://dx.doi.org/10.1016/j.cviu.2010.01.011"
            << std::endl;
  std::cout << "* Vision: Images, Signals and Neural Networks: Models of Neural Processing in Visual Perception "
  ...
  try {
    ...
    // if the last parameter is 'log', then activate log sampling (favour foveal vision and subsamples peripheral
    // vision)
    if(useLogSampling) {
    ...
    // save default retina parameters file in order to let you see this and maybe modify it and reload using method
    // "setup"
