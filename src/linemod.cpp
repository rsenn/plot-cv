...
private:
  static void
  cv_on_mouse(int a_event, int a_x, int a_y, int, void*) {
    m_event = a_event;
    m_x = a_x;
    m_y = a_y;
  }
...
main(int argc, char* argv[]) {
  ...
  for(;;) {
    ...
    // Perform matching
    std::vector<cv::linemod::cv::Match> matches;
    ...
    for(int i = 0; (i < (int)matches.size()) && (classes_visited < num_classes); ++i) {
      cv::linemod::cv::Match m = matches[i];
      ...
      if(visited.insert(m.class_id).second) {
        ...
        if(learn_online == true) {
          ...
          std::vector<Cvcv::Point> chain =
              maskFromTemplate(templates, num_modalities, cv::Point(m.x, m.y), color.size(), color_mask, display);
...
filterPlane(IplImage* ap_depth, std::vector<IplImage*>& a_masks, std::vector<Cvcv::Point>& a_chain, double f) {
  ...
  std::vector<Cvcv::Point> l_chain_std::vector;
  ...
  std::vector<cv::Point3d> lp_src_3Dpts(l_chain_std::vector.size());
  ...
  reprojectcv::Points(lp_src_3Dpts, lp_src_3Dpts, f);

  Cvcv::Mat* lp_pts = cvCreatecv::Mat((int)l_chain_std::vector.size(), 4, CV_32F);
  Cvcv::Mat* lp_v = cvCreatecv::Mat(4, 4, CV_32F);
  Cvcv::Mat* lp_w = cvCreatecv::Mat(4, 1, CV_32F);
  ...
  for(int l_i = 0; l_i < (int)l_chain_std::vector.size(); ++l_i) {
    float l_dist = l_n[0] * CV_MAT_ELEM(*lp_pts, float, l_i, 0) + l_n[1] * CV_MAT_ELEM(*lp_pts, float, l_i, 1) +
                   l_n[2] * CV_MAT_ELEM(*lp_pts, float, l_i, 2) + l_n[3] * CV_MAT_ELEM(*lp_pts, float, l_i, 3);

    if(fabs(l_dist) > l_max_dist)
      l_max_dist = l_dist;
  }
  // std::cerr << "plane: " << l_n[0] << ";" << l_n[1] << ";" << l_n[2] << ";" << l_n[3] << " maxdist: " << l_max_dist
  // << " end" << std::endl;
  ...
  Cvcv::Point* lp_chain = new Cvcv::Point[l_nn];

  for(int l_i = 0; l_i < l_nn; ++l_i) lp_chain[l_i] = a_chain[l_i];
  ...
  reprojectcv::Points(lp_dst_3Dpts, lp_dst_3Dpts, f);
  ...
  cvReleasecv::Mat(&lp_pts);
  cvReleasecv::Mat(&lp_w);
  cvReleasecv::Mat(&lp_v);
}

void
subtractPlane(const cv::Mat& depth, cv::Mat& mask, std::vector<Cvcv::Point>& chain, double f) {
