...
filterPlane(IplImage* ap_depth, std::vector<IplImage*>& a_masks, std::vector<Cvcv::Point>& a_chain, double f) {
  ...
  for(int l_i = 0; l_i < (int)l_chain_std::vector.size(); ++l_i) {
    float l_dist = l_n[0] * CV_MAT_ELEM(*lp_pts, float, l_i, 0) + l_n[1] * CV_MAT_ELEM(*lp_pts, float, l_i, 1) + l_n[2] * CV_MAT_ELEM(*lp_pts, float, l_i, 2) + l_n[3] * CV_MAT_ELEM(*lp_pts, float, l_i, 3);
