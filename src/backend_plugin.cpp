...
static inline FileSystemPath_t
toFileSystemPath(const std::string& p) {
  return p;
}
...
static inline void*
getSymbol_(LibHandle_t h, const char* symbolName) {
#if defined(_WIN32)
...
static inline LibHandle_t
libraryLoad_(const FileSystemPath_t& filename) {
#if defined(_WIN32)
#ifdef WINRT
...
public:
  ...
  void*
  getSymbol(const char* symbolName) const {
    if(!handle) {
      return 0;
    }
...
private:
  void
  libraryLoad(const FileSystemPath_t& filename) {
...
public:
  ...
  PluginBackend(const Ptr<DynamicLib>& lib) : lib_(lib), plugin_api_(NULL) {
    ...
    if(fn_init) {
      ...
      CV_LOG_INFO(NULL,
                  "Video I/O: plugin is incompatible, missing init function: '" << init_name
                                                                                << "', file: " << lib->getName());
    }
  }
  ...
  Ptr<IVideoWriter>
  createWriter(const std::string& filename, int fourcc, double fps, const cv::Size& sz, bool isColor) const CV_OVERRIDE;
};
...
#else
  CV_LOG_INFO(NULL,
              "VideoIO pluigin (" << baseName << "): glob is '" << plugin_expr << "', " << paths.size()
                                  << " location(s)");
...
void
PluginBackendFactory::loadPlugin() {
    ...
    try {
      ...
      if(pluginBackend && pluginBackend->plugin_api_) {
        if(pluginBackend->plugin_api_->captureAPI != id_) {
          CV_LOG_ERROR(NULL,
                       "Video I/O: plugin '" << pluginBackend->plugin_api_->api_header.api_description
      ...
      CV_LOG_WARNING(NULL,
                     "Video I/O: exception during plugin initialization: " << toPrintablePath(plugin) << ". SKIP");
    }
  }
}
...
public:
  ...
  ~PluginCapture() {
    ...
    if(CV_ERROR_OK != plugin_api_->Capture_release(capture_))
      CV_LOG_ERROR(NULL,
                   "Video I/O: Can't release capture by plugin '" << plugin_api_->api_header.api_description << "'");
    capture_ = NULL;
  }
  double
  getProperty(int prop) const CV_OVERRIDE {
    double val = -1;
  ...
  bool
  setProperty(int prop, double val) CV_OVERRIDE {
  ...
  retrieve_callback(
      int stream_idx, const unsigned char* data, int step, int width, int height, int cn, void* userdata) {
...
public:
  ...
  ~PluginWriter() {
    ...
    if(CV_ERROR_OK != plugin_api_->Writer_release(writer_))
      CV_LOG_ERROR(NULL,
                   "Video I/O: Can't release writer by plugin '" << plugin_api_->api_header.api_description << "'");
    writer_ = NULL;
  }
  double
  getProperty(int prop) const CV_OVERRIDE {
  ...
  write(cv::InputArray arr) CV_OVERRIDE {
    ...
    if(CV_ERROR_OK !=
       plugin_api_->Writer_write(writer_, img.data, (int)img.step[0], img.cols, img.rows, img.channels())) {
...
Ptr<IVideoCapture>
PluginBackend::createCapture(const std::string& filename) const {
  try {
    if(plugin_api_)
...
Ptr<IBackendFactory>
createPluginBackendFactory(VideoCaptureAPIs id, const char* baseName) {
