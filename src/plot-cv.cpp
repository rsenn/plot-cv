...
void
draw_lines(
    image_type& target, InputIterator start, InputIterator end, const cv::Scalar& color, int thickness = 1, int lineType = cv::LINE_8) {
...
process_raster(std::function<void(std::string, cv::Mat*)> display_image, int show_image) {
  ...
  corner_harris_detection(imgGrayscale,
                          [&](const point_type<int>& pt) { cv::circle(imgCanny, pt, 10, cv::Scalar(0, 255, 0, 255), 2, cv::LINE_8); });
  ...
  if(false) {
    ...
    for(size_t i = 0; i < lines.size(); ++i) {
      ...
      if(length > (length - range)) {
        ...
        */
        distances.clear();
        std::transform(adjacent_lines.begin(), adjacent_lines.end(), back_inserter(distances), [&line](Line<float>* l2) -> float {
        ...
        angleoffs.clear();
        std::transform(adjacent_lines.begin(), adjacent_lines.end(), back_inserter(angleoffs), [&line](Line<float>* l2) -> float {
        ...
        std::transform(adjacent_lines.begin(), adjacent_lines.end(), back_inserter(centers), [](Line<float>* line) -> point_type<int> {
          return line->center();
        });
