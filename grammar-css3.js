...
export function stylesheet(...args) {
  return wrap(seq(ws, any(seq(charset, any(choice(Comment, Space, Cdo, Cdc)))), any(seq(imports, any(choice(Comment, Space, Cdo, Cdc)))), any(seq(namespace, any(choice(Comment, Space, Cdo, Cdc)))), any(seq(nestedStatement, any(choice(Comment, Space, Cdo, Cdc))))), 'stylesheet')(...args);
}
...
export function imports(...args) {
  return wrap(choice(seq(Import, ws, choice(String, Uri), ws, mediaQueryList, token(';'), ws), seq(Import, ws, choice(String, Uri), ws, token(';'), ws), seq(Import, ws, choice(String, Uri), ws, mediaQueryList), seq(Import, ws, choice(String, Uri), ws)), 'imports')(...args);
}

export function namespace(...args) {
  return wrap(choice(seq(Namespace, ws, option(seq(namespacePrefix, ws)), choice(String, Uri), ws, token(';'), ws), seq(Namespace, ws, option(seq(namespacePrefix, ws)), choice(String, Uri), ws)), 'namespace')(...args);
}
...
export function simpleSelectorSequence(...args) {
  return wrap(choice(seq(choice(typeSelector, universal), any(choice(Hash, className, attrib, pseudo, negation))), many(choice(Hash, className, attrib, pseudo, negation))), 'simpleSelectorSequence')(...args);
}
...
export function attrib(...args) {
  return wrap(seq(token('['), ws, option(typeNamespacePrefix), ident, ws, option(seq(choice(PrefixMatch, SuffixMatch, SubstringMatch, token('='), Includes, DashMatch), ws, choice(ident, String), ws)), token(']')), 'attrib')(...args);
}
...
export function term(...args) {
  return wrap(choice(seq(number, ws), seq(percentage, ws), seq(dimension, ws), seq(String, ws), seq(UnicodeRange, ws), seq(ident, ws), variable, seq(Uri, ws), hexcolor, calc, func, seq(unknownDimension, ws), dxImageTransform), 'term')(...args);
}
...
export function any(...args) {
  return wrap(choice(seq(ident, ws), seq(number, ws), seq(percentage, ws), seq(dimension, ws), seq(unknownDimension, ws), seq(String, ws), seq(Uri, ws), seq(Hash, ws), seq(UnicodeRange, ws), seq(Includes, ws), seq(DashMatch, ws), seq(token(':'), ws), seq(Function, ws, any(choice(any, unused)), token(')'), ws), seq(token('('), ws, any(choice(any, unused)), token(')'), ws), seq(token('['), ws, any(choice(any, unused)), token(']'), ws)), 'any')(...args);
}
...
export function UnicodeRange(...args) {
  return wrap(choice(seq(regex(/[u|U]/g), token('+?'), option(token('?')), option(token('?')), option(token('?')), option(token('?')), option(token('?'))), seq(regex(/[u|U]/g), token('+'), Hex, option(token('?')), option(token('?')), option(token('?')), option(token('?')), option(token('?'))), seq(regex(/[u|U]/g), token('+'), Hex, Hex, option(token('?')), option(token('?')), option(token('?')), option(token('?'))), seq(regex(/[u|U]/g), token('+'), Hex, Hex, Hex, option(token('?')), option(token('?')), option(token('?'))), seq(regex(/[u|U]/g), token('+'), Hex, Hex, Hex, Hex, option(token('?')), option(token('?'))), seq(regex(/[u|U]/g), token('+'), Hex, Hex, Hex, Hex, Hex, option(token('?')))), 'UnicodeRange')(...args);
}
...
export function String(...args) {
  return wrap(choice(seq(token('"'), any(choice(invert(regex(/[\n\r\f\\"]/g)), seq(token('\\'), Newline), Nonascii, Escape)), token('"')), seq(token("'"), any(choice(invert(regex(/[\n\r\f\\']/g)), seq(token('\\'), Newline), Nonascii, Escape)), token("'"))), 'String')(...args);
}
