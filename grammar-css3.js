...
export function namespace(...args) {
  return wrap(
    choice(
      seq(Namespace, ws, option(seq(namespacePrefix, ws)), choice(String, Uri), ws, token(';'), ws),
      seq(Namespace, ws, option(seq(namespacePrefix, ws)), choice(String, Uri), ws)
    ),
    'namespace'
  )(...args);
}
...
export function mediaQuery(...args) {
  return wrap(
    choice(
      seq(option(choice(MediaOnly, Not)), ws, mediaType, ws, any(seq(And, ws, mediaExpression))),
      seq(mediaExpression, any(seq(And, ws, mediaExpression)))
    ),
    'mediaQuery'
  )(...args);
}
...
export function mediaExpression(...args) {
  return wrap(
    seq(token('('), ws, mediaFeature, option(seq(token(':'), ws, expr)), token(')'), ws),
    'mediaExpression'
  )(...args);
}
...
export function page(...args) {
  return wrap(
    seq(
      ...
      ws,
      option(declaration),
      any(seq(token(';'), ws, option(declaration))),
      token('}'),
      ws
    ),
    'page'
...
export function simpleSelectorSequence(...args) {
  return wrap(
    choice(
      seq(choice(typeSelector, universal), any(choice(Hash, className, attrib, pseudo, negation))),
      many(choice(Hash, className, attrib, pseudo, negation))
    ),
...
export function attrib(...args) {
  return wrap(
    seq(
      ...
      option(
        seq(
          choice(PrefixMatch, SuffixMatch, SubstringMatch, token('='), Includes, DashMatch),
          ws,
          choice(ident, String),
          ws
        )
...
export function expression(...args) {
  return wrap(
    many(seq(choice(Plus, Minus, Dimension, UnknownDimension, Number, String, ident), ws)),
    'expression'
  )(...args);
}
...
export function property(...args) {
  return wrap(
    choice(seq(ident, ws), seq(Variable, ws), seq(token('*'), ident), seq(token('_'), ident)),
    'property'
  )(...args);
}
...
export function ruleset(...args) {
  return wrap(
    choice(
      seq(selectorGroup, token('{'), ws, option(declarationList), token('}'), ws),
      seq(any(any), token('{'), ws, option(declarationList), token('}'), ws)
    ),
    'ruleset'
  )(...args);
}
...
export function declarationList(...args) {
  return wrap(
    seq(any(seq(token(';'), ws)), declaration, ws, any(seq(token(';'), ws, option(declaration)))),
    'declarationList'
  )(...args);
}
...
export function declaration(...args) {
  return wrap(
    choice(seq(property, token(':'), ws, expr, option(prio)), seq(property, token(':'), ws, value)),
    'declaration'
  )(...args);
}
...
export function block(...args) {
  return wrap(
    seq(
      token('{'),
      ws,
      any(choice(declarationList, nestedStatement, any, block, seq(atKeyword, ws), seq(token(';'), ws))),
      token('}'),
      ws
    ),
    'block'
...
export function supportsCondition(...args) {
  return wrap(
    choice(supportsNegation, supportsConjunction, supportsDisjunction, supportsConditionInParens),
    'supportsCondition'
  )(...args);
}
...
export function supportsConditionInParens(...args) {
  return wrap(
    choice(seq(token('('), ws, supportsCondition, ws, token(')')), supportsDeclarationCondition, generalEnclosed),
...
export function supportsConjunction(...args) {
  return wrap(
    seq(supportsConditionInParens, many(seq(ws, Space, ws, And, ws, Space, ws, supportsConditionInParens))),
    'supportsConjunction'
  )(...args);
}
...
export function supportsDisjunction(...args) {
  return wrap(
    seq(supportsConditionInParens, many(seq(ws, Space, ws, Or, ws, Space, ws, supportsConditionInParens))),
    'supportsDisjunction'
  )(...args);
}
...
export function calcSum(...args) {
  return wrap(
    seq(calcProduct, any(seq(Space, ws, choice(Plus, Minus), ws, Space, ws, calcProduct))),
    'calcSum'
  )(...args);
}
...
export function calcProduct(...args) {
  return wrap(
    seq(calcValue, any(choice(seq(token('*'), ws, calcValue), seq(token('/'), ws, number, ws)))),
    'calcProduct'
  )(...args);
}
...
export function fontFaceRule(...args) {
  return wrap(
    seq(
      ...
      token('{'),
      ws,
      option(fontFaceDeclaration),
      any(seq(token(';'), ws, option(fontFaceDeclaration))),
      token('}'),
      ws
    ),
    'fontFaceRule'
...
export function fontFaceDeclaration(...args) {
  return wrap(
    choice(seq(property, token(':'), ws, expr), seq(property, token(':'), ws, value)),
    'fontFaceDeclaration'
  )(...args);
}
...
export function keyframesRule(...args) {
  return wrap(
    seq(Keyframes, ws, Space, ws, ident, ws, token('{'), ws, keyframesBlocks, token('}'), ws),
    'keyframesRule'
  )(...args);
}
...
export function keyframesBlocks(...args) {
  return wrap(
    any(seq(keyframeSelector, token('{'), ws, option(declarationList), token('}'), ws)),
    'keyframesBlocks'
  )(...args);
}
...
export function keyframeSelector(...args) {
  return wrap(
    seq(choice(From, To, Percentage), ws, any(seq(Comma, ws, choice(From, To, Percentage), ws))),
    'keyframeSelector'
  )(...args);
}
...
export function counterStyle(...args) {
  return wrap(
    seq(CounterStyle, ws, ident, ws, token('{'), ws, option(declarationList), token('}'), ws),
    'counterStyle'
  )(...args);
}
...
export function fontFeatureValuesRule(...args) {
  return wrap(
    seq(FontFeatureValues, ws, fontFamilyNameList, ws, token('{'), ws, any(featureValueBlock), token('}'), ws),
    'fontFeatureValuesRule'
  )(...args);
}
...
export function featureValueBlock(...args) {
  return wrap(
    seq(
      ...
      token('{'),
      ws,
      option(featureValueDefinition),
      any(seq(ws, token(';'), ws, option(featureValueDefinition))),
      token('}'),
      ws
    ),
    'featureValueBlock'
...
export function Unicode(...args) {
  return wrap(
    seq(token('\\'), Hex, option(Hex), option(Hex), option(Hex), option(Hex), option(Hex), NewlineOrSpace),
    'Unicode'
  )(...args);
}
...
export function Comment(...args) {
  return wrap(
    seq(
      ...
      invert(any(token('*'))),
      many(token('*')),
      any(seq(invert(regex(/[\/*]/g)), invert(any(token('*'))), many(token('*')))),
      token('/')
    ),
    'Comment'
  )(...args);
...
export function ZeroToFourZeros(...args) {
  return wrap(
    seq(option(token('0')), option(token('0')), option(token('0')), option(token('0'))),
    'ZeroToFourZeros'
  )(...args);
}
...
export function A(...args) {
  return wrap(
    seq(seq(token('\\'), ZeroToFourZeros, choice(token('41'), token('61')), NewlineOrSpace), option()),
    'A'
  )(...args);
}
...
export function B(...args) {
  return wrap(
    seq(seq(token('\\'), ZeroToFourZeros, choice(token('42'), token('62')), NewlineOrSpace), option()),
    'B'
  )(...args);
}
...
export function C(...args) {
  return wrap(
    seq(seq(token('\\'), ZeroToFourZeros, choice(token('43'), token('63')), NewlineOrSpace), option()),
    'C'
  )(...args);
}
...
export function D(...args) {
  return wrap(
    seq(seq(token('\\'), ZeroToFourZeros, choice(token('44'), token('64')), NewlineOrSpace), option()),
    'D'
  )(...args);
}
...
export function E(...args) {
  return wrap(
    seq(seq(token('\\'), ZeroToFourZeros, choice(token('45'), token('65')), NewlineOrSpace), option()),
    'E'
  )(...args);
}
...
export function F(...args) {
  return wrap(
    seq(seq(token('\\'), ZeroToFourZeros, choice(token('46'), token('66')), NewlineOrSpace), option()),
    'F'
  )(...args);
}
...
export function G(...args) {
  return wrap(
    seq(
      choice(
        seq(token('\\'), ZeroToFourZeros, choice(token('47'), token('67')), NewlineOrSpace),
        token('\\g'),
        token('\\G')
      ),
      option()
...
export function H(...args) {
  return wrap(
    seq(
      choice(
        seq(token('\\'), ZeroToFourZeros, choice(token('48'), token('68')), NewlineOrSpace),
        token('\\h'),
        token('\\H')
      ),
      option()
...
export function I(...args) {
  return wrap(
    seq(
      choice(
        seq(token('\\'), ZeroToFourZeros, choice(token('49'), token('69')), NewlineOrSpace),
        token('\\i'),
        token('\\I')
      ),
      option()
...
export function K(...args) {
  return wrap(
    seq(
      choice(
        seq(token('\\'), ZeroToFourZeros, choice(token('4b'), token('6b')), NewlineOrSpace),
        token('\\k'),
        token('\\K')
      ),
      option()
...
export function L(...args) {
  return wrap(
    seq(
      choice(
        seq(token('\\'), ZeroToFourZeros, choice(token('4c'), token('6c')), NewlineOrSpace),
        token('\\l'),
        token('\\L')
      ),
      option()
...
export function M(...args) {
  return wrap(
    seq(
      choice(
        seq(token('\\'), ZeroToFourZeros, choice(token('4d'), token('6d')), NewlineOrSpace),
        token('\\m'),
        token('\\M')
      ),
      option()
...
export function N(...args) {
  return wrap(
    seq(
      choice(
        seq(token('\\'), ZeroToFourZeros, choice(token('4e'), token('6e')), NewlineOrSpace),
        token('\\n'),
        token('\\N')
      ),
      option()
...
export function O(...args) {
  return wrap(
    seq(
      choice(
        seq(token('\\'), ZeroToFourZeros, choice(token('4f'), token('6f')), NewlineOrSpace),
        token('\\o'),
        token('\\O')
      ),
      option()
...
export function P(...args) {
  return wrap(
    seq(
      choice(
        seq(token('\\'), ZeroToFourZeros, choice(token('50'), token('70')), NewlineOrSpace),
        token('\\p'),
        token('\\P')
      ),
      option()
...
export function Q(...args) {
  return wrap(
    seq(
      choice(
        seq(token('\\'), ZeroToFourZeros, choice(token('51'), token('71')), NewlineOrSpace),
        token('\\q'),
        token('\\Q')
      ),
      option()
...
export function R(...args) {
  return wrap(
    seq(
      choice(
        seq(token('\\'), ZeroToFourZeros, choice(token('52'), token('72')), NewlineOrSpace),
        token('\\r'),
        token('\\R')
      ),
      option()
...
export function S(...args) {
  return wrap(
    seq(
      choice(
        seq(token('\\'), ZeroToFourZeros, choice(token('53'), token('73')), NewlineOrSpace),
        token('\\s'),
        token('\\S')
      ),
      option()
...
export function T(...args) {
  return wrap(
    seq(
      choice(
        seq(token('\\'), ZeroToFourZeros, choice(token('54'), token('74')), NewlineOrSpace),
        token('\\t'),
        token('\\T')
      ),
      option()
...
export function U(...args) {
  return wrap(
    seq(
      choice(
        seq(token('\\'), ZeroToFourZeros, choice(token('55'), token('75')), NewlineOrSpace),
        token('\\u'),
        token('\\U')
      ),
      option()
...
export function V(...args) {
  return wrap(
    seq(
      choice(
        seq(token('\\'), ZeroToFourZeros, choice(token('56'), token('76')), NewlineOrSpace),
        token('\\v'),
        token('\\V')
      ),
      option()
...
export function W(...args) {
  return wrap(
    seq(
      choice(
        seq(token('\\'), ZeroToFourZeros, choice(token('57'), token('77')), NewlineOrSpace),
        token('\\w'),
        token('\\W')
      ),
      option()
...
export function X(...args) {
  return wrap(
    seq(
      choice(
        seq(token('\\'), ZeroToFourZeros, choice(token('58'), token('78')), NewlineOrSpace),
        token('\\x'),
        token('\\X')
      ),
      option()
...
export function Y(...args) {
  return wrap(
    seq(
      choice(
        seq(token('\\'), ZeroToFourZeros, choice(token('59'), token('79')), NewlineOrSpace),
        token('\\y'),
        token('\\Y')
      ),
      option()
...
export function Z(...args) {
  return wrap(
    seq(
      choice(
        seq(token('\\'), ZeroToFourZeros, choice(token('5a'), token('7a')), NewlineOrSpace),
        token('\\z'),
        token('\\Z')
      ),
      option()
...
export function FontRelative(...args) {
  return wrap(
    choice(seq(Number, E, M), seq(Number, E, X), seq(Number, C, H), seq(Number, R, E, M)),
    'FontRelative'
  )(...args);
}
...
export function ViewportRelative(...args) {
  return wrap(
    choice(seq(Number, V, W), seq(Number, V, H), seq(Number, V, M, I, N), seq(Number, V, M, A, X)),
    'ViewportRelative'
  )(...args);
}
...
export function Angle(...args) {
  return wrap(
    choice(seq(Number, D, E, G), seq(Number, R, A, D), seq(Number, G, R, A, D), seq(Number, T, U, R, N)),
    'Angle'
  )(...args);
}
...
export function Uri(...args) {
  return wrap(
    choice(
      seq(U, R, L, token('('), Whitespace, String, Whitespace, token(')')),
      seq(U, R, L, token('('), Whitespace, Url, Whitespace, token(')'))
    ),
    'Uri'
  )(...args);
}
...
export function String(...args) {
  return wrap(
    choice(
      seq(
        token('"'),
        any(choice(invert(regex(/[\n\r\f\\"]/g)), seq(token('\\'), Newline), Nonascii, Escape)),
        token('"')
      ),
      seq(
        token("'"),
...
export function VendorPrefix(...args) {
  return wrap(
    choice(
      seq(token('-'), M, O, Z, token('-')),
      seq(token('-'), W, E, B, K, I, T, token('-')),
      seq(token('-'), O, token('-'))
    ),
    'VendorPrefix'
  )(...args);
...
export function FontFeatureValues(...args) {
  return wrap(
    seq(token('@'), F, O, N, T, DashChar, F, E, A, T, U, R, E, DashChar, V, A, L, U, E, S),
    'FontFeatureValues'
  )(...args);
}
