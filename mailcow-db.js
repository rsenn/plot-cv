...
*/
  ...
  res = await q(`INSERT INTO mailbox (username,password) VALUES ${users.map(cols => `(${MySQL.valueString(...cols)})`).join(', ')};`);
  console.log('res =', res);
  ...
  res = await q(`SELECT * FROM mailbox WHERE username IN ('root','roman');`);
  console.log('res =', res);
  let ids = [];
