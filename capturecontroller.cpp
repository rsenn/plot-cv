...
CaptureWorker::doWork() {
  ...
  if(ok) {
    ...
    m_capture = new cv::VideoCapture(
        "rkcamsrc device=/dev/video0 io-mode=4 ! video/x-raw,format=NV12,width=640,height=480 ! videoconvert ! appsink",
        cv::CAP_GSTREAMER);
  }
...
const cv::Mat
CaptureController::frameRef() const {
  if(!m_worker)
    return cv::Mat();
...
cv::Mat
CaptureController::frameCopy() const {
  if(!m_worker)
    return cv::Mat();
  QReadLocker lock(m_lock);
  cv::Mat frame;
...
CaptureController::status() const {
  return m_status;
}

void
CaptureController::start(const QString& device) {
...
void
CaptureController::setStatus(CaptureController::Status status) {
  if(status == m_status)
    return;
  m_status = status;
  emit statusChanged();
