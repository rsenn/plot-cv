...
async function processFiles(...files) {
  ...
  for (let file of files) {
    ...
    let typedefs = [
      ...Util.filter(mainNodes, ([path, decl]) => decl.kind == 'TypedefDecl')
    ];
    ...
    let typenames = names(
      typedefs.filter(([path, decl]) => re.test(decl.name))
    );
    ...
    const ids = deep
      ...
      .map(({ path, value }) => [
        ...
        ...((n) => [n.id, n.kind])(
          deep.get(ast, path.slice(0, BasePathIndex(path)))
        ),
        value,
    ...
    console.log('ids:', ids);
    const refs = ids.map(
      ([path, refId, refKind, refLoc, declId, declKind, declLoc]) => [
        path,
...
async function AstDump(file, extraArgs) {
  ...
  const numErrors =
    errorLines.length &&
    +errorLines[errorLines.length - 1].replace(
      /.*\s([0-9]+)\serrors\sgenerated.*/g,
      '$1'
    );
