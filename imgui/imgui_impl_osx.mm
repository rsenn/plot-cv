...
// [BETA] Beta bindings, not well tested. If you want a portable application, prefer using the Glfw
// or SDL platform bindings on Mac.

// Issues:
// [ ] Platform: Keys are all generally very broken. Best using [event keycode] and not [event
// characters].. [ ] Platform: Mouse cursor shapes and visibility are not supported (see end of
// https://github.com/glfw/glfw/issues/427)
...
ImGui_ImplOSX_Init() {
  ...
  // values (optional) io.BackendFlags |= ImGuiBackendFlags_HasSetMousePos;          // We can honor
  // io.WantSetMousePos requests (optional, rarely used) io.BackendFlags |=
  // ImGuiBackendFlags_PlatformHasViewports;    // We can create multi-viewports on the Platform side
  // (optional) io.BackendFlags |= ImGuiBackendFlags_HasMouseHoveredViewport; // We can set
...
ImGui_ImplOSX_HandleEvent(NSEvent* event, NSView* view) {
  ...
  // FIXME: All the key handling is wrong and broken. Refer to GLFW's cocoa_init.mm and
  // cocoa_window.mm.
  if(event.type == NSEventTypeKeyDown) {
    ...
    for(int i = 0; i < len; i++) {
      ...
      // We must reset in case we're pressing a sequence of special keys while keeping the command
      // pressed
  ...
  if(event.type == NSEventTypeFlagsChanged) {
    ...
    // We must reset them as we will not receive any keyUp event if they where pressed with a
    // modifier
