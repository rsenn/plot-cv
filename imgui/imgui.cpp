...
/*
 ...
 draw list to display contents behind or over every other imgui windows.
...
#define IMGUI_DEBUG_NAV_SCORING                                                                    \
  0 // Display navigation scoring preview when hovering items. Display last moving direction matches
    // when holding CTRL
...
static void
SettingsHandlerWindow_ReadLine(ImGuiContext*, ImGuiSettingsHandler*, void* entry, const char* line);
...
ImFileOpen(const char* filename, const char* mode) {
  ...
  // We need a fopen() wrapper because MSVC/Windows fopen doesn't handle UTF-8 filenames. Converting
  // both strings from UTF-8 to wchar format (using a single allocation, because we can)
...
int
ImTextStrToUtf8(char* buf, int buf_size, const ImWchar* in_text, const ImWchar* in_text_end) {
  char* buf_out = buf;
...
void
ImGuiTextFilter::TextRange::split(char separator, ImVector<TextRange>* out) const {
  out->resize(0);
  const char* wb = b;
  const char* we = wb;
  while(we < e) {
...
ImGuiTextBuffer::appendfv(const char* fmt, va_list args) {
  ...
  int len = ImFormatStringV(NULL, 0, fmt, args); // FIXME-OPT: could do a first pass write attempt,
...
SetCursorPosYAndSetupDummyPrevLine(float pos_y, float line_height) {
  ...
  window->DC.PrevLineSize.y =
      (line_height -
       GImGui->Style.ItemSpacing.y); // If we end up needing more accurate data (to e.g. use
...
ImGuiListClipper::Step() {
  ...
  if(StepNo == 1) // Step 1: the clipper infer height from first element, calculate the actual range
                  // of elements to display, and position the cursor before the first element.
  {
    if(ItemsCount == 1) {
      ItemsCount = -1;
      return false;
  ...
  if(StepNo ==
     2) // Step 2: dummy step only required if an explicit items_height was passed to constructor or
...
void
ImGui::RenderArrow(ImVec2 p_min, ImGuiDir dir, float scale) {
  ImGuiContext& g = *GImGui;
  ...
  g.CurrentWindow->DrawList->AddTriangleFilled(center + a,
                                               center + b,
                                               center + c,
...
void
ImGui::SetNavID(ImGuiID id, int nav_layer) {
  ImGuiContext& g = *GImGui;
...
void
ImGui::SetNavIDWithRectRel(ImGuiID id, int nav_layer, const ImRect& rect_rel) {
  ImGuiContext& g = *GImGui;
...
ImGui::ItemAdd(const ImRect& bb, ImGuiID id, const ImRect* nav_bb_arg) {
  ...
  if(id != 0) {
    ...
    //  regular navigation requests unfortunately, but it is still limited to one window.
    //      it may not scale very well for windows with ten of thousands of item, but at least
    //      NavMoveRequest is only set on user interaction, aka maximum once a frame. We could early
    //      out with "if (is_clipped && !g.NavInitRequest) return false;" but when we wouldn't be
    //      able to reach unclipped widgets. This would work if user had explicit scrolling control
...
static ImDrawList*
GetForegroundDrawList(ImGuiWindow*) {
...
ImGui::UpdateHoveredWindowAndCaptureFlags() {
  ...
  // = dispatch mouse info to imgui + app)
  ...
  // false = dispatch keyboard info to imgui + app)
...
AddDrawListToDrawData(ImVector<ImDrawList*>* out_list, ImDrawList* draw_list) {
  ...
  // are drawing lots of stuff manually: A) Make sure you are coarse clipping, because ImDrawList
  // let all your vertices pass. You can use the Metrics window to inspect draw list contents. B) If
  // you need/want meshes with more than 64K vertices, uncomment the '#define ImDrawIdx unsigned
  // int' line in imconfig.h to set the index size to 4 bytes.
  //    You'll need to handle the 4-bytes indices to your renderer. For example, the OpenGL example
  //    code detect index size at compile-time by doing:
  ...
  // BeginChild()/EndChild() before reaching the 64K limit to split your draw commands in multiple
  // draw lists.
...
ImGui::EndFrame() {
  ...
  if(g.CurrentWindowStack.Size != 1) {
    if(g.CurrentWindowStack.Size > 1) {
      IM_ASSERT(g.CurrentWindowStack.Size == 1 && "Mismatched Begin/BeginChild vs End/EndChild "
  ...
  for(int n = 0; n < IM_ARRAYSIZE(windows_to_render_front_most); n++)
    if(windows_to_render_front_most[n] &&
       IsWindowActiveAndVisible(
           windows_to_render_front_most[n])) // NavWindowingTarget is always temporarily displayed
...
int
ImGui::CalcTypematicPressedRepeatAmount(float t,
                                        float t_prev,
                                        float repeat_delay,
                                        float repeat_rate) {
  if(t == 0.0f)
...
bool
ImGui::BeginChild(ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags extra_flags) {
  IM_ASSERT(id != 0);
...
static ImVec2
CalcSizeAfterConstraint(ImGuiWindow* window, ImVec2 new_size) {
  ImGuiContext& g = *GImGui;
...
static ImVec2
CalcSizeContents(ImGuiWindow* window) {
  if(window->Collapsed)
...
float
ImGui::GetWindowScrollMaxX(ImGuiWindow* window) {
  return ImMax(0.0f, window->SizeContents.x - (window->SizeFull.x - window->ScrollbarSizes.x));
}

float
ImGui::GetWindowScrollMaxY(ImGuiWindow* window) {
...
static void
ImGui::UpdateManualResize(ImGuiWindow* window,
                          const ImVec2& size_auto_fit,
                          int* border_held,
                          int resize_grip_count,
                          ImU32 resize_grip_col[4]) {
...
static void
ImGui::RenderOuterBorders(ImGuiWindow* window) {
  ImGuiContext& g = *GImGui;
...
ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags) {
  ...
  if(first_begin_of_the_frame) {
    ...
    // windows) We reset Size/SizeContents for reappearing popups/tooltips early in this function,
    ...
    if(dim_bg_for_modal || dim_bg_for_window_list) {
      const ImU32 dim_bg_col =
          GetColorU32(dim_bg_for_modal ? ImGuiCol_ModalWindowDimBg : ImGuiCol_NavWindowingDimBg,
                      g.DimBgRatio);
    ...
    // FIXME: window->ContentsRegionRect.Max is currently very misleading / partly faulty, but some
    ...
    if(!(flags & ImGuiWindowFlags_NoTitleBar)) {
      ...
      // FIXME: Refactor text alignment facilities along with RenderText helpers, this is too much
      // code..
    ...
    */
    ...
    // frame Note that if our window is collapsed we will end up with an inverted (~null) clipping
    ...
    // We fill last item data based on Title Bar, in order for IsItemHovered() and IsItemActive() to
    // be usable after Begin(). This is useful to allow creating context menus on title bar only,
    // etc.
...
void
ImGui::FocusPreviousWindowIgnoringOne(ImGuiWindow* ignore_window) {
  ImGuiContext& g = *GImGui;
...
ImGui::CalcItemWidth() {
  ...
  if(w < 0.0f) {
    // Align to a right-side limit. We include 1 frame padding in the calculation because this is
    // how the width is always used (we add 2 frame padding to it), but we could move that
...
struct ImGuiStyleVarInfo {
  ...
  ImU32 Offset;
  void*
  GetVarPtr(ImGuiStyle* style) const {
...
void
ImGui::SetWindowScrollX(ImGuiWindow* window, float new_scroll_x) {
...
void
ImGui::SetWindowScrollY(ImGuiWindow* window, float new_scroll_y) {
  window->DC.CursorMaxPos.y +=
...
ImGui::SetNextWindowContentSize(const ImVec2& size) {
  ...
  g.NextWindowData.ContentSizeVal =
      size; // In Begin() we will add the size of window decorations (title bar, menu etc.) to that
            // to form a SizeContents value.
...
ImGui::SetNextWindowFocus() {
  ...
  g.NextWindowData.FocusCond =
      ImGuiCond_Always; // Using a Cond member for consistency (may transition all of them to single
                        // flag set for fast Clear() op)
}
...
ImGui::EndGroup() {
  ...
  if(group_data.AdvanceCursor) {
    window->DC.CurrentLineTextBaseOffset = ImMax(
        ...
        group_data.BackupCurrentLineTextBaseOffset); // FIXME: Incorrect, we should grab the base
  ...
  // little more burden on individual widgets. (and if you grep for LastItemId you'll notice it is
...
// arise first.
void
ImGui::BeginTooltipEx(ImGuiWindowFlags extra_flags, bool override_previous_tooltip) {
  ImGuiContext& g = *GImGui;
  char window_name[16];
...
void
ImGui::OpenPopupEx(ImGuiID id) {
  ImGuiContext& g = *GImGui;
  ImGuiWindow* parent_window = g.CurrentWindow;
  int current_stack_size = g.BeginPopupStack.Size;
  ImGuiPopupRef popup_ref; // Tagged as new ref as Window will be set back to NULL if we write this
                           // into OpenPopupStack.
...
ImGui::ClosePopupsOverWindow(ImGuiWindow* ref_window) {
  ...
  if(ref_window) {
    ...
    for(; popup_count_to_keep < g.OpenPopupStack.Size; popup_count_to_keep++) {
      ...
      // Trim the stack if popups are not direct descendant of the reference window (which is often
      // the NavWindow)
      bool popup_or_descendent_has_focus = false;
      for(int m = popup_count_to_keep; m < g.OpenPopupStack.Size && !popup_or_descendent_has_focus;
          m++)
...
void
ImGui::ClosePopupToLevel(int remaining, bool apply_focus_to_window_under) {
  ...
  // g.OpenPopupStack[remaining-1].Window etc. we should find the highest root window that is behind
  // the popups we are closing. The current code will set focus to the parent of the popup window
  // which is incorrect. It rarely manifested until now because UpdateMouseMovingWindowNewFrame()
  // would call FocusWindow() again on the clicked window, leading to a chain of focusing A (clicked
  // window) then B (parent window of the popup) then A again. However if the clicked window has the
  // _NoMove flag set we would be left with B focused. For now, we have disabled this path when
  // called from ClosePopupsOverWindow() because the users of ClosePopupsOverWindow() don't need to
  // alter focus anyway, but we should inspect and fix this properly.
...
bool
ImGui::BeginPopupContextWindow(const char* str_id, int mouse_button, bool also_over_items) {
  if(!str_id)
...
ImRect
ImGui::GetWindowAllowedExtentRect(ImGuiWindow*) {
...
void
ImGui::NavMoveRequestTryWrapping(ImGuiWindow* window, ImGuiNavMoveFlags move_flags) {
  ImGuiContext& g = *GImGui;
...
static void
ImGui::NavSaveLastChildNavWindow(ImGuiWindow* nav_window) {
...
ImGui::GetNavInputAmount(ImGuiNavInput n, ImGuiInputReadMode mode) {
  ...
  if(mode == ImGuiInputReadMode_Repeat)
    return (float)CalcTypematicPressedRepeatAmount(t,
                                                   t - g.IO.DeltaTime,
...
static void
NavScrollToBringItemIntoView(ImGuiWindow* window, const ImRect& item_rect) {
...
static float
ImGui::NavUpdatePageUpPageDown(int allowed_dir_flags) {
  ImGuiContext& g = *GImGui;
    ...
    if(page_up_held != page_down_held) // If either (not both) are pressed
    {
      if(window->DC.NavLayerActiveMask == 0x00 && window->DC.NavHasScroll) {
        ...
        if(IsKeyPressed(g.IO.KeyMap[ImGuiKey_PageUp], true)) {
          ...
          nav_scoring_rect_offset_y = +page_offset_y;
          g.NavMoveDir =
              ImGuiDir_Up; // Because our scoring rect is offset, we intentionally request the
                           // opposite direction (so we can always land on the last item)
...
static int
FindWindowFocusIndex(ImGuiWindow* window) // FIXME-OPT O(N)
{
  ImGuiContext& g = *GImGui;
  for(int i = g.WindowsFocusOrder.Size - 1; i >= 0; i--)
    if(g.WindowsFocusOrder[i] == window)
...
ImGui::NavUpdateWindowing() {
  ...
  if(g.NavWindowingTarget && g.NavInputSource == ImGuiInputSource_NavGamepad) {
    ...
    // the window was merely rendered front-most)
...
static float
OffsetNormToPixels(const ImGuiColumnsSet* columns, float offset_norm) {
  return offset_norm * (columns->MaxX - columns->MinX);
}

static float
PixelsToOffsetNorm(const ImGuiColumnsSet* columns, float offset) {
...
static float
GetDraggedColumnOffset(ImGuiColumnsSet* columns, int column_index) {
...
static float
GetColumnWidthEx(ImGuiColumnsSet* columns, int column_index, bool before_resize = false) {
  if(column_index < 0)
    column_index = columns->Current;
...
ImGui::SetColumnOffset(int column_index, float offset) {
  ...
  if(!(columns->Flags & ImGuiColumnsFlags_NoForceWithinWindow))
    offset =
        ImMin(offset, columns->MaxX - g.Style.ColumnsMinSpacing * (columns->Count - column_index));
...
static ImGuiColumnsSet*
FindOrAddColumnsSet(ImGuiWindow* window, ImGuiID id) {
...
void
ImGui::BeginColumns(const char* str_id, int columns_count, ImGuiColumnsFlags flags) {
  ImGuiContext& g = *GImGui;
  ImGuiWindow* window = GetCurrentWindow();
...
ImGui::EndColumns() {
  ...
  if(!(columns->Flags & ImGuiColumnsFlags_NoBorder) && !window->SkipItems) {
    ...
    for(int n = 1; n < columns->Count; n++) {
      ...
      // Draw column (we clip the Y boundaries CPU side because very long triangles are mishandled
      // by some GPU drivers.)
      const ImU32 col =
...
ImGui::LogFinish() {
  ...
  LogText(IM_NEWLINE);
  switch(g.LogType) {
    case ImGuiLogType_TTY: fflush(g.LogFile); break;
    case ImGuiLogType_File: fclose(g.LogFile); break;
...
static void*
SettingsHandlerWindow_ReadOpen(ImGuiContext*, ImGuiSettingsHandler*, const char* name) {
...
static void
SettingsHandlerWindow_ReadLine(ImGuiContext*,
                               ImGuiSettingsHandler*,
                               void* entry,
                               const char* line) {
...
static void
SettingsHandlerWindow_WriteAll(ImGuiContext* imgui_ctx,
                               ImGuiSettingsHandler* handler,
                               ImGuiTextBuffer* buf) {
  ...
  for(int i = 0; i != g.SettingsWindows.Size; i++) {
    ...
    if(const char* p = strstr(name, "###")) // Skip to the "###" marker if any. We don't skip past
                                            // to match the behavior of GetID()
      name = p;
...
ImGui::ShowMetricsWindow(bool* p_open) {
  ...
  struct Funcs {
    ...
    static void
    NodeWindows(ImVector<ImGuiWindow*>& windows, const char* label) {
    ...
    static void
    NodeWindow(ImGuiWindow* window, const char* label) {
    ...
    static void
    NodeTabBar(ImGuiTabBar* tab_bar) {
  ...
  if(g.IO.KeyCtrl && show_window_begin_order) {
    for(int n = 0; n < g.Windows.Size; n++) {
      ...
      fg_draw_list->AddRectFilled(window->Pos,
