...
struct ImGuiWindowSettings;        // Storage for window settings stored in .ini file (we keep one of those
                                   // even if the actual window wasn't instanced during this session)
...
static inline ImU32
ImHash(const void* data, int size, ImU32 seed = 0) {
...
static inline float
ImFmod(float x, float y) {
  return fmodf(x, y);
}
static inline double
ImFmod(double x, double y) {
  return fmod(x, y);
}
static inline float
ImCos(float x) {
...
enum ImGuiItemStatusFlags_ {
  ...
  ImGuiItemStatusFlags_ToggledSelection =
      1 << 3 // Set when Selectable(), TreeNode() reports toggling a selection. We can't report
             // "Selected" because reporting the change allows us to handle clipping with less issues.
...
struct ImGuiContext {
  ...
  bool ActiveIdHasBeenPressed;  // Track whether the active id led to a press (this is to allow
  ...
  ImGuiID NavJustMovedToMultiSelectScopeId; // Just navigated to this select scope id (result of a
                                            // successfully MoveRequest).
  ImGuiID NavNextActivateId;                // Set by ActivateItem(), queued until next frame.
  ImGuiInputSource NavInputSource;          // Keyboard or Gamepad mode? THIS WILL ONLY BE None or NavGamepad
                                            // or NavKeyboard.
  ...
  ImGuiNavMoveResult NavMoveResultOther;           // Best move request candidate within NavWindow's flattened hierarchy
  ...
  int FocusRequestCurrCounterAll;      // Any item being requested for focus, stored as an index (we on
  ...
  ImDrawList ForegroundDrawList; // Last draw list to be rendered. This is where we the render software
  ...
  ImVector<ImGuiWindowSettings> SettingsWindows;   // ImGuiWindow .ini settings entries (parsed from the last loaded .ini file
                                                   // and maintained on saving)
...
struct IMGUI_API ImGuiWindowTempData {
  ...
  ImVec2 CursorMaxPos;   // Used to implicitly calculate the size of our contents, always growing during
                         // the frame. Turned into window->SizeContents at the beginning of next frame
...
struct IMGUI_API ImGuiWindow {
  ...
  ImVec2 SizeFullAtLastBegin;  // Copy of SizeFull at the end of Begin. This is the reference value
  ...
  ImVec2 SetWindowPosPivot;               // store window pivot for positioning. ImVec2(0,0) when positioning from
                                          // top-left corner; ImVec2(0.5f,0.5f) for centering; ImVec2(1,1) for
                                          // bottom right.
  ...
  ImRect OuterRectClipped;   // = WindowRect just after setup in Begin(). == window->Rect() for root
                             // window.
  ImRect InnerMainRect, InnerClipRect;
  ImRect ContentsRegionRect; // FIXME: This is currently confusing/misleading. Maximum visible content
                             // position ~~ Pos + (SizeContentsExplicit ? SizeContentsExplicit : Size -
  ...
  ImGuiWindow* ParentWindow;                   // If we are a child _or_ popup window, this is pointing to our parent.
                                               // Otherwise NULL.
...
enum ImGuiTabBarFlagsPrivate_ {
  ...
  ImGuiTabBarFlags_SaveSettings = 1 << 22 // FIXME: Settings are handled by the docking system, this only request the tab bar to
                                          // mark settings dirty when reordering tabs
};

enum ImGuiTabItemFlagsPrivate_ {
  ImGuiTabItemFlags_NoCloseButton = 1 << 20 // Store whether p_open is set or not, which we need to
...
struct ImGuiTabBar {
  ...
  ImGuiTabBar();
  int
  GetTabOrder(const ImGuiTabItem* tab) const {
    return Tabs.index_from_ptr(tab);
  }
  const char*
  GetTabName(const ImGuiTabItem* tab) const {
...
inline bool
IsNavInputDown(ImGuiNavInput n) {
  return GImGui->IO.NavInputs[n] > 0.0f;
}
inline bool
IsNavInputPressed(ImGuiNavInput n, ImGuiInputReadMode mode) {
  return GetNavInputAmount(n, mode) > 0.0f;
}
inline bool
IsNavInputPressedAnyOfTwo(ImGuiNavInput n1, ImGuiNavInput n2, ImGuiInputReadMode mode) {
...
IMGUI_API void VerticalSeparator(); // Vertical separator, for menu bars (use current line height). Not exposed
...
IMGUI_API bool DragBehaviorT(
    ImGuiDataType data_type, T* v, float v_speed, T v_min, T v_max, const char* format, float power, ImGuiDragFlags flags);
template<typename T, typename SIGNED_T, typename FLOAT_T>
