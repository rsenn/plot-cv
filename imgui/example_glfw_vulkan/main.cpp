...
SetupVulkan(const char** extensions, uint32_t extensions_count) {
  ...
  // Select GPU
  {
    ...
    // e.g. VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU if available, or with the greatest memory
    // available, etc. for sake of simplicity we'll just take the first one, assuming it has a
...
SetupVulkanWindow(ImGui_ImplVulkanH_Window* wd, VkSurfaceKHR surface, int width, int height) {
  ...
  ImGui_ImplVulkanH_CreateWindow(g_Instance,
                                 g_PhysicalDevice,
                                 g_Device,
                                 wd,
                                 g_QueueFamily,
...
static void
FrameRender(ImGui_ImplVulkanH_Window* wd) {
  VkResult err;
...
static void
glfw_resize_callback(GLFWwindow*, int w, int h) {
