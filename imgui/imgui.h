...
ImColor)
// Draw List API (ImDrawCallback, ImDrawCmd, ImDrawIdx, ImDrawVert, ImDrawChannel, ImDrawListFlags,
...
struct ImDrawChannel;              // Temporary storage for ImDrawList ot output draw commands out of order, used
                                   // by ImDrawList::ChannelsSplit()
...
typedef unsigned short ImWchar;   // A single U16 character for keyboard input/display. We encode them
...
IMGUI_API void EndFrame();           // ends the ImGui frame. automatically called by Render(), you likely don't need to call
                                     ...
                                     // call EndFrame() but you'll have wasted CPU already! If you don't need to render,
...
IMGUI_API void ShowDemoWindow(bool* p_open = NULL);  // create demo/test window (previously called ShowTestWindow). demonstrate
                                                     // most ImGui features. call this to learn about the library! try to make
                                                     // it always available in your application!
IMGUI_API void ShowAboutWindow(bool* p_open = NULL); // create about window. display Dear ImGui version, credits
                                                     // and build/system information.
IMGUI_API void
ShowMetricsWindow(bool* p_open = NULL); // create metrics/debug window. display Dear ImGui internals: draw commands (with
...
IMGUI_API bool
IsWindowHovered(ImGuiHoveredFlags flags = 0); // is current window hovered (and typically: not blocked by a popup/modal)?
...
IMGUI_API void SetNextWindowSize(const ImVec2& size, ImGuiCond cond = 0); // set next window size. set axis to 0.0f to force an
...
IMGUI_API void
SetNextWindowContentSize(const ImVec2& size); // set next window content size (~ enforce the range of scrollbars). not
                                              // including window decorations (title bar, menu bar, etc.). set an axis to
...
IMGUI_API void SetWindowCollapsed(bool collapsed, ImGuiCond cond = 0); // (not recommended) set current window collapsed state.
                                                                       // prefer using SetNextWindowCollapsed().
IMGUI_API void SetWindowFocus();                // (not recommended) set current window to be focused / front-most.
...
InvisibleButton(const char* str_id,
                const ImVec2& size); // button behavior without the visuals, frequently useful to build custom behaviors
...
IMGUI_API bool SliderFloat(const char* label,
                           ...
                           float v_max,
                           const char* format = "%.3f",
                           float power = 1.0f); // adjust format to decorate the value with a prefix or a suffix for
...
IMGUI_API bool VSliderFloat(
    const char* label, const ImVec2& size, float* v, float v_min, float v_max, const char* format = "%.3f", float power = 1.0f);
...
Selectable(const char* label,
           ...
           const ImVec2& size = ImVec2(0, 0)); // "bool selected" carry the selection state (read-only). Selectable()
...
IMGUI_API bool BeginPopupContextVoid(const char* str_id = NULL,
                                     int mouse_button = 1); // helper to open and begin popup when clicked
                                                            // in void (where there are no imgui windows).
IMGUI_API bool
BeginPopupModal(const char* name,
                bool* p_open = NULL,
                ImGuiWindowFlags flags = 0); // modal dialog (regular window with title bar, block interactions behind
...
IMGUI_API void CloseCurrentPopup();                        // close the popup we have begin-ed into. clicking on a MenuItem
                                                           // or Selectable automatically close the current popup.
...
IMGUI_API float GetColumnOffset(int column_index = -1); // get position of column line (in pixels, from the left side of the
...
IMGUI_API void
SetTabItemClosed(const char* tab_or_docked_window_label); // notify TabBar or Docking system of a closed tab/window ahead
...
IMGUI_API bool
IsMousePosValid(const ImVec2* mouse_pos = NULL); // by convention we use (-FLT_MAX,-FLT_MAX) to denote that there is no mouse
IMGUI_API ImVec2 GetMousePos();                  // shortcut to ImGui::GetIO().MousePos provided by user, to be
                                                 // consistent with other calls
IMGUI_API ImVec2 GetMousePosOnOpeningCurrentPopup(); // retrieve backup of mouse position at the time of opening
...
IMGUI_API ImGuiMouseCursor GetMouseCursor(); // get desired cursor type, reset in ImGui::NewFrame(), this is updated during the
...
IMGUI_API void CaptureKeyboardFromApp(
    bool want_capture_keyboard_value = true); // attention: misleading name! manually override io.WantCaptureKeyboard flag next
                                              ...
                                              // NewFrame() call.
IMGUI_API void
CaptureMouseFromApp(bool want_capture_mouse_value = true); // attention: misleading name! manually override io.WantCaptureMouse
...
IMGUI_API const char*
SaveIniSettingsToMemory(size_t* out_ini_size = NULL); // return a zero-terminated string with the .ini data which you can save
...
enum ImGuiWindowFlags_ {
  ...
  // [Internal]
  ImGuiWindowFlags_NavFlattened = 1 << 23, // [BETA] Allow gamepad/keyboard navigation to cross over parent border to this child
...
enum ImGuiTabItemFlags_ {
  ...
  ImGuiTabItemFlags_UnsavedDocument =
      1 << 0,                             // Append '*' to title without affecting the ID, as a convenience to avoid using the
                                          // ### operator. Also: tab is selected on closure and closure is deferred by one frame
...
enum ImGuiFocusedFlags_ {
  ...
  ImGuiFocusedFlags_AnyWindow = 1 << 2,  // IsWindowFocused(): Return true if any window is focused. Important: If you are
...
enum ImGuiDragDropFlags_ {
  ...
  ImGuiDragDropFlags_SourceNoPreviewTooltip = 1 << 0, // By default, a successful call to BeginDragDropSource opens a tooltip so
                                                      // you can display a preview or description of the source contents. This
                                                      // flag disable this behavior.
  ImGuiDragDropFlags_SourceNoDisableHover = 1
                                            << 1, // By default, when dragging we clear data so that IsItemHovered() will return
  ...
  ImGuiDragDropFlags_SourceAllowNullID = 1 << 3, // Allow items such as Text(), Image() that have no unique identifier to be
  ...
  ImGuiDragDropFlags_SourceExtern =
      1 << 4, // External source (from outside of imgui), won't attempt to read current item/window
  ...
  ImGuiDragDropFlags_AcceptPeekOnly =
      ImGuiDragDropFlags_AcceptBeforeDelivery | ImGuiDragDropFlags_AcceptNoDrawDefaultRect // For peeking ahead and inspecting
...
enum ImGuiConfigFlags_ {
  ...
  ImGuiConfigFlags_NavEnableGamepad =
      1 << 1, // Master gamepad navigation enable flag. This is mostly to instruct your imgui back-end
              // to fill io.NavInputs[]. Back-end also needs to set ImGuiBackendFlags_HasGamepad.
  ImGuiConfigFlags_NavEnableSetMousePos = 1 << 2, // Instruct navigation to move the mouse cursor. May be useful on TV/console
                                                  ...
                                                  // io.WantSetMousePos requests in your binding, otherwise ImGui will react as
                                                  // if the mouse is jumping around back and forth.
  ImGuiConfigFlags_NavNoCaptureKeyboard = 1 << 3, // Instruct navigation to not set the io.WantCaptureKeyboard flag when
                                                  // io.NavActive is set.
  ...
  ImGuiConfigFlags_NoMouseCursorChange = 1 << 5, // Instruct back-end to not alter mouse cursor shape and visibility. Use if the
...
enum ImGuiBackendFlags_ {
  ...
  ImGuiBackendFlags_HasSetMousePos = 1 << 2 // Back-end supports io.WantSetMousePos requests to reposition the OS mouse position
...
enum ImGuiColorEditFlags_ {
  ...
  ImGuiColorEditFlags_NoInputs = 1 << 5,  //              // ColorEdit, ColorPicker: disable inputs sliders/text widgets (e.g.
  ...
  ImGuiColorEditFlags_AlphaPreviewHalf = 1 << 18, //              // ColorEdit, ColorPicker, ColorButton: display half opaque /
                                                  //              half checkerboard, instead of opaque.
  ImGuiColorEditFlags_HDR = 1 << 19,        //              // (WIP) ColorEdit: Currently only disable 0.0f..1.0f limits in RGBA
...
struct ImNewDummy {};
inline void*
operator new(size_t, ImNewDummy, void* ptr) {
  return ptr;
}
inline void
operator delete(void*, ImNewDummy, void*) {
...
template<typename T> struct ImVector {
  ...
  inline T*
  erase(const T* it, const T* it_last) {
...
struct ImGuiStyle {
  ...
  float ChildRounding;           // Radius of child window corners rounding. Set to 0.0f to have rectangular
                                 // windows.
  float ChildBorderSize;         // Thickness of border around child windows. Generally set to 0.0f or 1.0f.
                                 // (Other values are not well tested and more CPU/GPU costly).
  ...
  float FrameBorderSize;         // Thickness of border around frames. Generally set to 0.0f or 1.0f. (Other
...
struct ImGuiIO {
  ...
  bool ConfigMacOSXBehaviors;             // = defined(__APPLE__) // OS X style: Text editing cursor movement
                                          ...
                                          // Multi-selection in lists uses Cmd/Super instead of Ctrl (was called
  ...
  bool ConfigWindowsMoveFromTitleBarOnly; // = false       // [BETA] Set to true to only allow
                                          // moving windows when clicked+dragged from the title bar.
  ...
  void* ImeWindowHandle; // = NULL           // (Windows) Set this to your HWND to get automatic IME
                         // cursor positioning.
  ...
  bool WantTextInput;           // Mobile/console: when io.WantTextInput is true, you may display an on-screen
                                // keyboard. This is set by ImGui when it wants textual keyboard input to
                                // happen (e.g. when a InputText widget is active).
  bool WantSetMousePos;         // MousePos has been altered, back-end should reposition mouse on next frame.
  ...
  bool NavActive;               // Directional navigation is currently allowed (will handle ImGuiKey_NavXXX events)
...
struct ImGuiInputTextCallbackData {
  ...
  ImWchar EventChar;  // Character input                      // Read-write   // [CharFilter] Replace
                      // character with another one, or set to zero to drop. return 1 is equivalent
                      // to setting EventChar=0;
  ImGuiKey EventKey;  // Key pressed (Up/Down/TAB)            // Read-only    // [Completion,History]
  char* Buf;          // Text buffer                          // Read-write   // [Resize] Can replace pointer
                      // / [Completion,History,Always] Only write to pointed data, don't replace the actual
                      // pointer!
...
struct ImGuiPayload {
  ...
  bool
  IsDataType(const char* type) const {
...
static inline ImDrawList*
GetOverlayDrawList() {
  return GetForegroundDrawList();
}
// OBSOLETED in 1.66 (from Sep 2018)
static inline void
SetScrollHere(float center_ratio = 0.5f) {
...
CalcItemRectClosestPoint(const ImVec2& pos, bool on_edge = false, float outward = 0.f) {
  IM_UNUSED(on_edge);
  IM_UNUSED(outward);
  IM_ASSERT(0);
  return pos;
...
struct ImDrawCmd {
  unsigned int ElemCount;      // Number of indices (multiple of 3) to be rendered as triangles. Vertices are
...
struct ImDrawList {
  ...
  ImVector<ImDrawCmd> CmdBuffer;  // Draw commands. Typically 1 command = 1 GPU draw call, unless the
  ...
  IMGUI_API void PushClipRect(
      ...
      bool intersect_with_current_clip_rect = false); // Render-level scissoring. This is passed down to your render function
  ...
  inline void
  PathStroke(ImU32 col, bool closed, float thickness = 1.0f) {
...
struct ImDrawData {
  ...
  ImDrawList** CmdLists;   // Array of ImDrawList* to render. The ImDrawList are owned by ImGuiContext
...
struct ImFontConfig {
  ...
  float SizePixels;             //          // Size in pixels for rasterizer (more or less maps to the resulting
                                //          font height).
  ...
  bool MergeMode;               // false    // Merge into previous ImFont, so you can combine multiple inputs font
                                // into one ImFont (e.g. ASCII font + icons + Japanese glyphs). You may want to
                                // use GlyphOffset.y when merge font of different heights.
  ...
  float RasterizerMultiply;     // 1.0f     // Brighten (>1.0f) or darken (<1.0f) font output. Brightening
                                // small fonts may be a good workaround to make them more readable.
...
struct ImFontAtlas {
  ...
  IMGUI_API ImFont* AddFontFromMemoryTTF(
      ...
      float size_pixels,
      const ImFontConfig* font_cfg = NULL,
      const ImWchar* glyph_ranges = NULL); // Note: Transfer ownership of 'ttf_data' to ImFontAtlas! Will be deleted after
  ...
  IMGUI_API void ClearTexData();           // Clear output texture data (CPU side). Saves RAM once the texture
  ...
  // You can request arbitrary rectangles to be packed into the atlas, for your own purposes. After
  // calling Build(), you can query the rectangle position and render your pixels. You can also
  // request your rectangles to be mapped as font glyph (given a font + Unicode point), so you can
  ...
  const CustomRect*
  GetCustomRectByIndex(int index) const {
    if(index < 0)
      return NULL;
...
struct ImFont {
  ...
  float
  GetCharAdvance(ImWchar c) const {
