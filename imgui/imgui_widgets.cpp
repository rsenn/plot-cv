...
static void
DataTypeApplyOp(ImGuiDataType data_type, int op, void* output, void* arg_1, const void* arg_2);
...
ImGui::CloseButton(ImGuiID id, const ImVec2& pos, float radius) {
  ...
  if(hovered)
    window->DrawList->AddCircleFilled(center,
                                      ImMax(2.0f, radius),
                                      GetColorU32(held ? ImGuiCol_ButtonActive
...
ImGuiID
ImGui::GetScrollbarID(ImGuiWindow* window, ImGuiAxis axis) {
...
ImGui::Scrollbar(ImGuiAxis axis) {
  ...
  if(held && allow_interaction && grab_h_norm < 1.0f) {
    ...
    // of SizeContents and before setting up our starting position
...
ImGui::NewLine() {
  ...
  if(window->DC.CurrentLineSize.y >
     0.0f) // In the event that we are on a line with items that is smaller that FontSize high, we
           // will preserve its height.
    ItemSize(ImVec2(0, 0));
  else
...
PatchFormatStringFloatToInt(const char* fmt) {
  ...
  if(fmt_end > fmt_start && fmt_end[-1] == 'f') {
    ...
    return g.TempBuffer;
#else
    IM_ASSERT(0 && "DragInt(): Invalid format string!"); // Old versions used a default parameter of
...
static void
DataTypeApplyOp(ImGuiDataType data_type, int op, void* output, void* arg1, const void* arg2) {
  IM_ASSERT(op == '+' || op == '-');
  switch(data_type) {
...
// mess..
static bool
DataTypeApplyOpFromText(const char* buf,
                        const char* initial_value_buf,
                        ImGuiDataType data_type,
                        void* data_ptr,
                        const char* format) {
...
ImGui::DragScalar(const char* label,
  ...
  if(focus_requested || (hovered && (g.IO.MouseClicked[0] || g.IO.MouseDoubleClicked[0])) ||
...
ImGui::SliderScalar(const char* label,
  ...
  const bool focus_requested = FocusableItemRegister(window, id);
  const bool hovered = ItemHoverable(frame_bb, id);
  if(focus_requested || (hovered && g.IO.MouseClicked[0]) || g.NavActivateId == id ||
...
bool
ImGui::InputScalarAsWidgetReplacement(const ImRect& bb,
                                      ImGuiID id,
                                      const char* label,
                                      ImGuiDataType data_type,
                                      void* data_ptr,
  ...
  if(value_changed)
    return DataTypeApplyOpFromText(
        data_buf, g.InputTextState.InitialTextA.Data, data_type, data_ptr, NULL);
  return false;
}
...
ImGui::InputTextEx(const char* label,
  ...
  if(g.ActiveId == id) {
    ...
    // ImGuiInputTextFlags_EnterReturnsTrue would fail. Also this allows the user to use InputText()
    // with ImGuiInputTextFlags_EnterReturnsTrue without maintaining any user-side storage.
    ...
    if(render_selection) {
      ...
      for(const ImWchar* p = text_selected_begin; p < text_selected_end;) {
        ...
        if(rect_pos.y < clip_rect.y) {
          ...
          // FIXME-OPT: Could use this when wchar_t are 16-bits p = p ? p + 1 : text_selected_end;
...
void
ImGui::RenderColorRectWithAlphaCheckerboard(ImVec2 p_min,
                                            ImVec2 p_max,
                                            ImU32 col,
                                            float grid_step,
                                            ImVec2 grid_off,
...
ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size) {
  ...
  ImRect bb_inner = bb;
  float off = -0.75f; // The border (using Col_FrameBg) tends to look off when color is near-opaque
...
ImGui::SetColorEditOptions(ImGuiColorEditFlags flags) {
  ...
  IM_ASSERT(
      ImIsPowerOfTwo(flags & ImGuiColorEditFlags__DataTypeMask)); // Check only 1 option is selected
  IM_ASSERT(
...
ImGui::TreeNodeBehavior(ImGuiID id,
  ...
  // (Ideally we'd want to add a flag for the user to specify if we want the hit test to be done up
  // to the right side of the content or not)
  ...
  if(!is_leaf) {
    if(pressed) {
      ...
      if(flags & ImGuiTreeNodeFlags_OpenOnArrow)
        toggled |=
            IsMouseHoveringRect(interact_bb.Min,
...
bool
ImGui::CollapsingHeader(const char* label, bool* p_open, ImGuiTreeNodeFlags flags) {
  ImGuiWindow* window = GetCurrentWindow();
  if(window->SkipItems)
    return false;
...
ImGui::Selectable(const char* label,
  ...
  RenderTextClipped(
      bb_inner.Min, bb_inner.Max, label, NULL, &label_size, style.SelectableTextAlign, &bb);
  if(flags & ImGuiSelectableFlags_Disabled)
    PopStyleColor();
...
float
ImGuiMenuColumns::DeclColumns(float w0,
                              float w1,
...
ImGui::BeginMenu(const char* label, bool enabled) {
  ...
  if(window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
    ...
    float w =
        window->MenuColumns.DeclColumns(label_size.x,
                                        0.0f,
  ...
  {
    // Implement http://bjk5.com/post/44698559168/breaking-down-amazons-mega-dropdown to avoid using
    ...
    if(g.HoveredWindow == window && g.OpenPopupStack.Size > g.BeginPopupStack.Size &&
      ...
      if(ImGuiWindow* next_window = g.OpenPopupStack[g.BeginPopupStack.Size].Window) {
        ...
        // window->DrawList->PushClipRectFullScreen(); window->DrawList->AddTriangleFilled(ta, tb,
        // tc, moving_within_opened_triangle ? IM_COL32(0,128,0,128) : IM_COL32(128,0,0,128));
        // window->DrawList->PopClipRect(); // Debug
      }
    }

    want_close =
        (menu_is_open && !hovered && g.HoveredWindow == window && g.HoveredIdPreviousFrame != 0 &&
...
ImGui::TabBarLayout(ImGuiTabBar* tab_bar) {
  ...
  for(int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++) {
    ...
    tab->WidthContents =
        TabItemCalcSize(tab_name, (tab->Flags & ImGuiTabItemFlags_NoCloseButton) ? false : true).x;
...
ImGui::BeginTabItem(const char* label, bool* p_open, ImGuiTabItemFlags flags) {
  ...
  if(ret && !(flags & ImGuiTabItemFlags_NoPushId)) {
    ...
    g.CurrentWindow->IDStack.push_back(
        tab->ID); // We already hashed 'label' so push into the ID stack directly instead of doing
  ...
  if(label_size.x > text_ellipsis_clip_bb.GetWidth()) {
    ...
    float label_size_clipped_x =
        g.Font
            ...
            .x;
    if(label_end == label &&
       label_end < label_display_end) // Always display at least 1 character if there's no room for
                                      // character + ellipsis
    {
