...
// Original code by @vuhdo (Aleksei Skriabin). Improvements by @mikesart. Maintained and v0.60+ by
// @ocornut.
...
// tweaks.
// - v0.51: (2017/08/26) cleanup, optimizations, support for ImFontConfig::RasterizerFlags,
// ImFontConfig::RasterizerMultiply.
// - v0.52: (2017/09/26) fixes for imgui internal changes.
// - v0.53: (2017/10/22) minor inconsequential change to match change in master (removed an
...
// - v0.60: (2019/01/10) re-factored to match big update in STB builder. fixed texture height waste.
// fixed redundant glyphs when merging. support for glyph padding.
...
//  output. The default imgui styles will be impacted by this change (alpha values will need
//  tweaking).
...
struct GlyphInfo {
  ...
  FT_Int
      OffsetY; // The distance from the origin to the top of the glyph. This is usually a value < 0.
  float AdvanceX; // The distance from the origin to the origin of the next glyph. This is usually a
                  // value > 0.
};
...
struct FontInfo {
  ...
  float LineSpacing; // The baseline-to-baseline distance. Note that it usually is larger than the
                     ...
                     // using this distance. Think of it as a value the designer of the font finds
                     // appropriate.
  float LineGap;     // The spacing in pixels between one row's descent and the next row's ascent.
  float MaxAdvanceWidth; // This field gives the maximum horizontal cursor advance for all glyphs in
                         // the font.
};
...
struct FreeTypeFont {
  bool InitFont(FT_Library ft_library,
                ...
                unsigned int extra_user_flags); // Initialize from an external data buffer. Doesn't
...
FreeTypeFont::BlitGlyph(const FT_Bitmap* ft_bitmap,
  ...
  if(multiply_table == NULL) {
    for(uint32_t y = 0; y < h; y++, src += src_pitch, dst += dst_pitch) memcpy(dst, src, w);
  } else {
...
struct ImFontBuildDstDataFT {
  ...
  int GlyphsHighest;
  int GlyphsCount;
  ImBoolVector GlyphsSet; // This is used to resolve collision when multiple sources are merged into
                          // a same destination font.
};

bool
ImFontAtlasBuildWithFreeType(FT_Library ft_library, ImFontAtlas* atlas, unsigned int extra_flags) {
  ...
  for(int src_i = 0; src_i < src_tmp_array.Size; src_i++) {
    ...
    for(const ImWchar* src_range = src_tmp.SrcRanges; src_range[0] && src_range[1]; src_range += 2)
      for(int codepoint = src_range[0]; codepoint <= src_range[1]; codepoint++) {
        if(dst_tmp.GlyphsSet.GetBit(codepoint)) // Don't overwrite existing glyphs. We could make
  ...
  for(int src_i = 0; src_i < src_tmp_array.Size; src_i++) {
    ...
    for(int glyph_i = 0; glyph_i < src_tmp.GlyphsCount; glyph_i++) {
      ...
      dst_font->AddGlyph(
          (ImWchar)src_glyph.Codepoint, x0, y0, x1, y1, u0, v0, u1, v1, char_advance_x_mod);
    }
...
static void*
ImFreeTypeDefaultAllocFunc(size_t size, void* user_data) {
  IM_UNUSED(user_data);
  return IM_ALLOC(size);
}
static void
ImFreeTypeDefaultFreeFunc(void* ptr, void* user_data) {
  IM_UNUSED(user_data);
  IM_FREE(ptr);
}
...
bool
ImGuiFreeType::BuildFontAtlas(ImFontAtlas* atlas, unsigned int extra_flags) {
