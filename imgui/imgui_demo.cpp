...
ImGui::ShowDemoWindow(bool* p_open) {
  ...
  ImGui::PushItemWidth(ImGui::GetFontSize() *
                       -12); // Use fixed width for labels (by passing a negative value), the rest
  ...
  if(ImGui::CollapsingHeader("Configuration")) {
    ...
    if(ImGui::TreeNode("Configuration##2")) {
      ...
      if(io.ConfigFlags & ImGuiConfigFlags_NoMouse) // Create a way to restore this flag otherwise
                                                    // we could be stuck completely!
      {
      ...
      ImGui::SameLine();
      HelpMarker(
          "Instruct Dear ImGui to render a mouse cursor for you. Note that a mouse cursor rendered "
    ...
    if(ImGui::TreeNode("Backend Flags")) {
      HelpMarker("Those flags are set by the back-ends (imgui_impl_xxx files) to specify their "
                 "capabilities.");
    ...
    if(ImGui::TreeNode("Capture/Logging")) {
      ImGui::TextWrapped(
          "The logging API redirects all text output so you can easily capture the content of a "
          "window or a block. Tree nodes can be automatically expanded.");
      HelpMarker("Try opening any of the contents below in this window and then click one of the "
                 "\"Log To\" button.");
      ImGui::LogButtons();
      ImGui::TextWrapped("You can also call ImGui::LogText() to output directly to the log without "
                         "a visual output.");
...
ShowDemoWindowWidgets() {
  ...
  if(ImGui::TreeNode("Basic")) {
...

    {
      ...
      // See "Combo" section for examples of how to use the more complete BeginCombo()/EndCombo()
      // api.
...

    {
      ...
      HelpMarker(
          ...
          "facility if you need to wire InputText() to a dynamic string type. See "
  ...
  if(ImGui::TreeNode("Trees")) {
    ...
    if(ImGui::TreeNode("Advanced, with Selectable nodes")) {
      ...
      static int selection_mask =
          (1
           << 2); // Dumb representation of what may be user-side selection state. You may carry
                  // selection state inside or outside your objects in whatever format you see fit.
      int node_clicked = -1; // Temporary storage of what node we have clicked to process selection
                             // at the end of the loop. May be a pointer to your own node type, etc.
      ImGui::PushStyleVar(
          ImGuiStyleVar_IndentSpacing,
          ImGui::GetFontSize() *
              3); // Increase spacing to differentiate leaves from expanded contents.
      for(int i = 0; i < 6; i++) {
        // Disable the default open on single-click behavior and pass in Selected flag according to
        // our selection state.
      ...
      if(node_clicked != -1) {
        // Update selection state. Process outside of tree loop to avoid visual inconsistencies
        // during the clicking-frame.
        if(ImGui::GetIO().KeyCtrl)
          selection_mask ^= (1 << node_clicked); // CTRL+click to toggle
        else // if (!(selection_mask & (1 << node_clicked))) // Depending on selection behavior you
             // want, this commented bit preserve selection when clicking on item that is part of the
             // selection
  ...
  if(ImGui::TreeNode("Text")) {
    ...
    if(ImGui::TreeNode("Word Wrapping")) {
      ...
      ImGui::TextWrapped("This text should automatically wrap on the edge of the window. The "
                         "current implementation for text wrapping follows simple rules suitable "
      ...
      ImGui::Text("The lazy dog is a good dog. This paragraph is made to fit within %.0f pixels. "
    ...
    if(ImGui::TreeNode("UTF-8 Text")) {
      ...
      // (Needs a suitable font, try Noto, or Arial Unicode, or M+ fonts. Read misc/fonts/README.txt
      // for details.)
      // - From C++11 you can use the u8"my text" syntax to encode literal strings as UTF-8
      // - For earlier compiler, you may be able to encode your sources as UTF-8 (e.g. Visual Studio
      // save your file as 'UTF-8 without signature')
      // - FOR THIS DEMO FILE ONLY, BECAUSE WE WANT TO SUPPORT OLD COMPILERS, WE ARE *NOT* INCLUDING
      // RAW UTF-8 CHARACTERS IN THIS SOURCE FILE.
      //   Instead we are encoding a few strings with hexadecimal constants. Don't do this in your
      //   application! Please use u8"text in any language" in your application!
      ...
      ImGui::TextWrapped("CJK text will only appears if the font was loaded with the appropriate "
                         "CJK character ranges. Call io.Font->AddFontFromFileTTF() manually to "
                         "load extra character ranges. Read misc/fonts/README.txt for details.");
  ...
  if(ImGui::TreeNode("Images")) {
    ...
    // Here we are grabbing the font texture because that's the only one we have access to inside
    // the demo code. Remember that ImTextureID is just storage for whatever you want it to be, it
    // is essentially a value that will be passed to the render function inside the ImDrawCmd
    // structure. If you use one of the default imgui_impl_XXXX.cpp renderer, they all have comments
    // at the top of their file to specify what they expect to be stored in ImTextureID. (for
    // example, the imgui_impl_dx11.cpp renderer expect a 'ID3D11ShaderResourceView*' pointer. The
    // imgui_impl_glfw_gl3.cpp renderer expect a GLuint OpenGL texture identifier etc.) If you
    // decided that ImTextureID = MyEngineTexture*, then you can pass your MyEngineTexture* pointers
    // to ImGui::Image(), and gather width/height through your own functions, etc. Using
    // ShowMetricsWindow() as a "debugger" to inspect the draw data that are being passed to your
    // render will help you debug issues if you are confused about this. Consider using the
    // lower-level ImDrawList::AddImage() API, via ImGui::GetWindowDrawList()->AddImage().
  ...
  if(ImGui::TreeNode("Combo")) {
    ...
    if(ImGui::BeginCombo("combo 1", item_current, flags)) // The second parameter is the label
                                                          // previewed before opening the combo.
    {
      for(int n = 0; n < IM_ARRAYSIZE(items); n++) {
        ...
        if(is_selected)
          ImGui::SetItemDefaultFocus(); // Set the initial focus when opening the combo (scrolling +
                                        // for keyboard navigation support in the upcoming
                                        // navigation branch)
      }
      ImGui::EndCombo();
    }
    ...
    struct FuncHolder {
      static bool
      ItemGetter(void* data, int idx, const char** out_str) {
  ...
  if(ImGui::TreeNode("Selectables")) {
    ...
    // - The one taking "bool selected" as a read-only selection information. When Selectable() has
    // been clicked is returns true and you can alter selection state accordingly.
    // - The one taking "bool* p_selected" as a read-write selection information (convenient in some
    // cases) The earlier is more flexible, as in real application your selection may be stored in a
    // different manner (in flags within objects, as an external list, etc).
    if(ImGui::TreeNode("Basic")) {
    ...
    if(ImGui::TreeNode("Grid")) {
      ...
      for(int i = 0; i < 4 * 4; i++) {
        ...
        if(ImGui::Selectable("Sailor", &selected[i], 0, ImVec2(50, 50))) {
          // Note: We _unnecessarily_ test for both x/y and i here only to silence some static
          // analyzer. The second part of each test is unnecessary.
          int x = i % 4;
          int y = i / 4;
          if(x > 0) {
            selected[i - 1] ^= 1;
    ...
    if(ImGui::TreeNode("Alignment")) {
      HelpMarker("Alignment applies when a selectable is larger than its text content.\nBy "
                 "default, Selectables uses style.SelectableTextAlign but it can be overriden on a "
  ...
  if(ImGui::TreeNode("Text Input")) {
    if(ImGui::TreeNode("Multi-line Text Input")) {
      ...
      HelpMarker("You can use the ImGuiInputTextFlags_CallbackResize facility if you need to wire "
                 "InputTextMultiline() to a dynamic string type. See misc/cpp/imgui_stdlib.h for "
    ...
    if(ImGui::TreeNode("Resize Callback")) {
      // If you have a custom string type you would typically create a ImGui::InputText() wrapper
      // than takes your type as input. See misc/cpp/imgui_stdlib.h and .cpp for an implementation
      // of this using std::string.
      HelpMarker("Demonstrate using ImGuiInputTextFlags_CallbackResize to wire your resizable "
  ...
  if(ImGui::TreeNode("Plots Widgets")) {
    ...
    // FIXME: This is rather awkward because current plot API only pass in indices. We probably want
    // an API passing floats and user provide sample rate/count.
    struct Funcs {
      static float
      Sin(void*, int i) {
        return sinf(i * 0.1f);
    ...
    // Typically we would use ImVec2(-1.0f,0.0f) to use all available width, or ImVec2(width,0.0f)
    // for a specified width. ImVec2(0.0f,0.0f) uses ItemWidth.
  ...
  if(ImGui::TreeNode("Color/Picker Widgets")) {
    ...
    ImGui::SameLine();
    HelpMarker("With the ImGuiColorEditFlags_NoInputs flag you can hide all the slider/text "
    ...
    HelpMarker("ColorEdit defaults to displaying RGB inputs if you don't specify a display mode, "
               ...
               "displaying RGB+HSV+Hex if you don't specify a display mode.\n\nYou can change the "
    ...
    if(!alpha)
      flags |= ImGuiColorEditFlags_NoAlpha; // This is by default if you call ColorPicker3() instead
                                            // of ColorPicker4()
    if(alpha_bar)
    ...
    HelpMarker(
        ...
        "Push/Pop functions because you can force options on a per-widget basis if needed, and the "
        "user can change non-forced ones with the options menu.\nWe don't have a getter to avoid "
...
// hence the /2 below.
    ...
    ImGui::DragScalar(
        "drag float ^2", ImGuiDataType_Float, &f32_v, 0.005f, &f32_zero, &f32_one, "%f", 2.0f);
    ImGui::SameLine();
    HelpMarker("You can use the 'power' parameter to increase tweaking precision on one side of "
               "the range.");
    ImGui::DragScalar(
        "drag double", ImGuiDataType_Double, &f64_v, 0.0005f, &f64_zero, NULL, "%.10f grams", 1.0f);
    ...
    ImGui::SliderScalar(
        "slider float low^2", ImGuiDataType_Float, &f32_v, &f32_zero, &f32_one, "%.10f", 2.0f);
    ImGui::SliderScalar(
    ...
    ImGui::SliderScalar("slider double low",
                        ...
                        1.0f);
    ImGui::SliderScalar(
        "slider double low^2", ImGuiDataType_Double, &f64_v, &f64_zero, &f64_one, "%.10f", 2.0f);
    ImGui::SliderScalar(
        "slider double high", ImGuiDataType_Double, &f64_v, &f64_lo_a, &f64_hi_a, "%e grams", 1.0f);
  ...
  if(ImGui::TreeNode("Querying Status (Active/Focused/Hovered etc.)")) {
    ...
    // flags can be combined. (because BulletText is an item itself and that would affect the output
    // of IsItemHovered() we pass all state in a single call to simplify the code).
...
ShowDemoWindowLayout() {
  ...
  if(ImGui::TreeNode("Child windows")) {
    ...
    // - Using SetCursorPos() to position the child window (because the child window is an item from
    // the POV of the parent window)
    ...
    // item from the POV of the parent window)
    //   See "Widgets" -> "Querying Status (Active/Focused/Hovered etc.)" section for more details
    //   about this.
    {
  ...
  if(ImGui::TreeNode("Basic Horizontal Layout")) {
    ...
    // Manually wrapping (we should eventually provide this as an automatic layout feature, but for
  ...
  if(ImGui::TreeNode("Tabs")) {
    ...
    if(ImGui::TreeNode("Advanced & Close Button")) {
      ...
      // Passing a bool* to BeginTabItem() is similar to passing one to Begin(): the underlying bool
      // will be set to false when the tab is closed.
      if(ImGui::BeginTabBar("MyTabBar", tab_bar_flags)) {
        for(int n = 0; n < IM_ARRAYSIZE(opened); n++)
  ...
  if(ImGui::TreeNode("Groups")) {
    HelpMarker("Using ImGui::BeginGroup()/EndGroup() to layout items. BeginGroup() basically locks "
  ...
  if(ImGui::TreeNode("Text Baseline Alignment")) {
    HelpMarker("This is testing the vertical alignment that gets applied on text to keep it "
               "aligned with widgets. Lines only composed of text or \"small\" widgets fit in less "
    ...
    ImGui::AlignTextToFramePadding(); // If your line starts with text, call this to align it to
                                      // upcoming widgets.
  ...
  if(ImGui::TreeNode("Scrolling")) {
    ...
    for(int i = 0; i < 5; i++) {
      ...
      ImGui::BeginGroup();
      ImGui::Text("%s",
                  i == 0 ? "Top" : i == 1 ? "25%" : i == 2 ? "Center" : i == 3 ? "75%" : "Bottom");
  ...
  if(ImGui::TreeNode("Horizontal Scrolling")) {
    ...
    for(int line = 0; line < lines; line++) {
      ...
      // manipulating the cursor position yourself, aka using SetCursorPos/SetCursorScreenPos to
    ...
    if(scroll_x_delta != 0.0f) {
      ImGui::BeginChild(
          "scrolling"); // Demonstrate a trick: you can use Begin to set yourself in the context of
  ...
  if(ImGui::TreeNode("Clipping")) {
    ...
    ImGui::TextWrapped("On a per-widget basis we are occasionally clipping text CPU-side if it "
                       "won't fit in its frame. Otherwise we are doing coarser clipping + passing "
                       "a scissor rectangle to the renderer. The system is designed to try "
                       "minimizing both execution and CPU/GPU rendering cost.");
...
ShowDemoWindowPopups() {
  ...
  if(ImGui::TreeNode("Popups")) {
    ...
    // (If you want to show the current selection inside the Button itself, you may want to build a
    // string using the "###" operator to preserve a constant ID with a variable label)
  ...
  if(ImGui::TreeNode("Context menus")) {
    ...
    // For more advanced uses you may want to replicate and cuztomize this code. This the comments
    // inside BeginPopupContextItem() implementation.
    ...
    // BeginPopupContextItem(). BeginPopupContextItem() will use the last item ID as the popup ID.
    // In addition here, we want to include your editable label inside the button label. We use the
  ...
  if(ImGui::TreeNode("Modals")) {
    ImGui::TextWrapped("Modal windows are like popups but the user cannot close them by clicking "
                       "outside the window.");
  ...
  if(ImGui::TreeNode("Menus inside a regular window")) {
    ...
    // menu from the parent of the various popup menus above. To do so we are encloding the items in
    // a PushID()/PopID() block to make them two different menusets. If we don't, opening any popup
    // above and hovering our menu here would open it. This is because once a menu is active, we
    // allow to switch to a sibling menu by just hovering on it, which is the desired behavior for
    // regular menus.
    ImGui::PushID("foo");
...
ShowDemoWindowMisc() {
  ...
  if(ImGui::CollapsingHeader("Inputs, Navigation & Focus")) {
    ...
    if(ImGui::TreeNode("Dragging")) {
      ...
      // Drag operations gets "unlocked" when the mouse has moved past a certain threshold (the
      // default threshold is stored in io.MouseDragThreshold) You can request a lower or higher
...
ImGui::ShowStyleEditor(ImGuiStyle* ref) {
  // You can pass in a reference ImGuiStyle structure to compare to, revert to and save to (else it
  // compares to an internally stored reference)
  ImGuiStyle& style = ImGui::GetStyle();
  ...
  if(ImGui::Button("Revert Ref"))
    style = *ref;
  ImGui::SameLine();
  HelpMarker("Save/Revert in local non-persistent storage. Default Colors definition are not "
  ...
  if(ImGui::BeginTabBar("##tabs", ImGuiTabBarFlags_None)) {
    if(ImGui::BeginTabItem("Sizes")) {
      ...
      ImGui::SameLine();
      HelpMarker("Adjust if you cannot see the edges of your screen (e.g. on a TV where scaling "
    ...
    if(ImGui::BeginTabItem("Colors")) {
      ...
      for(int i = 0; i < ImGuiCol_COUNT; i++) {
        ...
        if(memcmp(&style.Colors[i], &ref->Colors[i], sizeof(ImVec4)) != 0) {
          ...
          // misc/fonts/README.txt about using icon fonts. It's really easy and super convenient!
      ...
      for(int i = 0; i < atlas->Fonts.Size; i++) {
        ...
        if(font_details_opened) {
          ...
          HelpMarker("Note than the default embedded font is NOT meant to be scaled.\n\nFont are "
                     "currently rendered into bitmaps at a given size at the time of building the "
                     "atlas. You may oversample them to get some flexibility with scaling. You can "
                     "also render at multiple sizes and select which one to use at "
                     "runtime.\n\n(Glimmer of hope: the atlas system should hopefully be rewritten "
                     "in the future to make scaling more natural and automatic.)");
...
struct ExampleAppConsole {
  ...
  ExampleAppConsole() {
    ...
    Commands.push_back("CLASSIFY"); // "classify" is only here to provide an example of "C"+[tab]
  ...
  Draw(const char* title, bool* p_open) {
    ...
    ImGui::TextWrapped("This example implements a console with basic coloring, completion and "
                       "history. A more elaborate implementation may want to store entries along "
    ...
    // you only want raw text you can use ImGui::TextUnformatted(log.begin(), log.end()); NB- if you
    // have thousands of entries this approach may be too inefficient and may require user-side
    // clipping to only process visible items. You can seek and display only the lines that are
    // visible using the ImGuiListClipper helper, if your elements are evenly spaced and you have
    // cheap random access to the elements. To use the clipper we could replace the 'for (int i = 0;
    // i < Items.Size; i++)' loop with:
    //     ImGuiListClipper clipper(Items.Size);
    //     while (clipper.Step())
    //         for (int i = clipper.DisplayStart; i < clipper.DisplayEnd; i++)
    // However, note that you can not use this code as is if a filter is active because it breaks
    // the 'cheap random-access' property. We would need random-access on the post-filtered list. A
    // typical application wanting coarse clipping and filtering may want to pre-compute an array of
    // indices that passed the filtering test, recomputing this array when user changes the filter,
    // and appending newly elements as they are inserted. This is left as a task to the user until
    // we can manage to improve this example code! If your items are of variable size you may want
    // to implement code similar to what ImGuiListClipper does. Or split your data into fixed height
...
ShowExampleAppPropertyEditor(bool* p_open) {
  ...
  HelpMarker("This example shows how you may implement a property editor using two columns.\nAll "
             "objects/fields data are dummies here.\nRemember that in many simple cases, you can "
             "use ImGui::SameLine(xxx) to position\nyour cursor horizontally instead of using the "
             "Columns() API.");
  ...
  struct funcs {
    static void
    ShowDummyObject(const char* prefix, int uid) {
      ImGui::PushID(uid); // Use object uid as identifier. Most commonly you could also use the
                          // object pointer as a base ID.
      ImGui::AlignTextToFramePadding(); // Text and Tree nodes are less high than regular widgets,
                                        // here we add vertical spacing to make the tree lines equal
                                        // high.
...
ShowExampleAppAutoResize(bool* p_open) {
  ...
  static int lines = 10;
  ImGui::Text("Window will resize every-frame to the size of its content.\nNote that you probably "
              "don't want to query the window size to\noutput your content because that would "
...
ShowExampleAppCustomRendering(bool* p_open) {
  ...
  if(ImGui::BeginTabBar("##TabBar")) {
    ...
    if(ImGui::BeginTabItem("Canvas")) {
      ...
      // us to use IsItemHovered() But you can also draw directly and poll mouse/keyboard by
    ...
    if(ImGui::BeginTabItem("BG/FG draw lists")) {
      ...
      if(draw_bg)
        ImGui::GetBackgroundDrawList()->AddCircle(
            window_center, window_size.x * 0.6f, IM_COL32(255, 0, 0, 200), 32, 10 + 4);
      if(draw_fg)
        ImGui::GetForegroundDrawList()->AddCircle(
            window_center, window_size.y * 0.6f, IM_COL32(0, 255, 0, 200), 32, 10);
      ImGui::EndTabItem();
    }
