...
std::string const vert_shader_source = "#version 150\n"
                                       "in vec3 vertex;\n"
                                       "in vec2 texCoord0;\n"
...
std::string const frag_shader_source = "#version 150\n"
...
// attribute indices
enum { VERTICES = 0, TEX_COORDS };

// uniform indices
enum { MVP_MATRIX = 0, FRAME_TEX };
...
void
initializeAppData(AppData* data) {
  data->fmt_ctx = NULL;
  data->stream_idx = -1;
...
void
clearAppData(AppData* data) {
  if(data->av_frame)
    av_free(data->av_frame);
  if(data->gl_frame)
    av_free(data->gl_frame);
  if(data->packet)
    av_free(data->packet);
  if(data->codec_ctx)
...
bool
buildShader(std::string const& shader_source, GLuint& shader, GLenum type) {
...
main(int argc, char* argv[]) {
  ...
  int size = avpicture_get_size(PIX_FMT_RGB24, data.codec_ctx->width, data.codec_ctx->height);
  uint8_t* internal_buffer = (uint8_t*)av_malloc(size * sizeof(uint8_t));
  avpicture_fill((AVPicture*)data.gl_frame, internal_buffer, PIX_FMT_RGB24, data.codec_ctx->width, data.codec_ctx->height);
  ...
  glVertexAttribPointer(data.attribs[TEX_COORDS], 2, GL_FLOAT, GL_FALSE, 20, BUFFER_OFFSET(12));
  ...
  unsigned char elem[6] = {0, 1, 2, 0, 2, 3};
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, data.codec_ctx->width, data.codec_ctx->height, 0, GL_RGB, GL_UNSIGNED_BYTE, NULL);
